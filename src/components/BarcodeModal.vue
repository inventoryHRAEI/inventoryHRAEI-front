      ...
      <div v-if="visible" class="barcode-modal-overlay" @click.self="close">
        ...
        <div class="preview-panel">
          <div class="preview-toggle">
            <button :class="{ active: previewMode==='barcode' }" @click="previewMode='barcode'">Barcode</button>
        ...
        <div class="barcode-svg-wrapper">
          ...
          <svg v-show="!loading && !bwipActive && previewMode==='barcode'" ref="barcodeSvg" class="barcode-svg"></svg>
          <canvas v-show="!loading && bwipActive && previewMode==='barcode'" ref="barcodeCanvas" class="barcode-canvas"></canvas>

          <!-- QR preview -->
          <img v-if="!loading && previewMode==='qr' && qrDataUrl && !previewError" :src="qrDataUrl" class="qr-preview" />
        ...
        <div class="barcode-modal-actions">
          <button class="barcode-modal-download" @click="downloadBarcodePng" :disabled="!displayCode" aria-label="Descargar código de barras como PNG">Descargar PNG</button>
          <button class="barcode-modal-download" @click="downloadBarcodeWithUrl" :disabled="!displayCode" aria-label="Descargar QR con URL">Descargar QR (URL)</button>
...
async function generateQrPreview() {
  ...
  try {
    ...
    // Prefer public HTTPS base from cloudflare tunnel; fall back to dev-hosts only if cloudflare not present
    let publicBase = null
    try {
    ...
    if (!publicBase) {
      // Try dev hosts for an https host
      try {
    ...
    if (!publicBase) {
      alert('Necesitas iniciar el túnel Cloudflare para generar códigos QR públicos. Ejecuta "npm run dev:cloudflare" y vuelve a intentarlo.')
      loading.value = false
      return
    }

    const fullUrl = `${publicBase}/op/testing-biomedical?scan=${encodeURIComponent(code)}`
...
async function downloadBarcodeWithUrl() {
  ...
  const fullUrl = `${baseUrl}/op/testing-biomedical?scan=${encodeURIComponent(code)}`
...
.barcode-svg-wrapper svg, .barcode-svg-wrapper canvas { display:block }
.barcode-skeleton {
  width: 520px;
  height: 160px;
  max-width: 86vw;
...
.barcode-modal-secondary:disabled{ opacity:.4; cursor:not-allowed }
.barcode-empty {
  position: absolute;
...
.barcode-fade-enter-active, .barcode-fade-leave-active {
  transition: opacity 0.25s;
}
.barcode-fade-enter-from, .barcode-fade-leave-to {
  opacity: 0;
}
...
.barcode-thickness-label{ color: rgba(255,255,255,0.75); font-weight:600 }
.barcode-thickness-options{ display:flex; gap:8px }
.thick-btn{ background: rgba(255,255,255,0.04); color: rgba(255,255,255,0.9); border: 1px solid rgba(255,255,255,0.04); padding:6px 10px; border-radius:8px; cursor:pointer }
.thick-btn.active{ background: linear-gradient(135deg,#60a5fa,#3b82f6); color:#fff; border-color: rgba(59,130,246,0.6) }

/* Preview toggle */
.preview-toggle{ display:flex; gap:8px; justify-content:center; margin-bottom:8px }
.preview-toggle button{ padding:6px 12px; border-radius:8px; background: rgba(255,255,255,0.03); color: rgba(255,255,255,0.9); border:1px solid rgba(255,255,255,0.04); cursor:pointer }
.preview-toggle button.active{ background: linear-gradient(135deg,#60a5fa,#3b82f6); color:#fff; border-color: rgba(59,130,246,0.6) }
.qr-preview{ width: 200px; height: 200px; object-fit: contain; background: #fff; border-radius:8px; }
.barcode-caption{ color: rgba(255,255,255,0.85); font-weight:600; margin-top:8px }
.skeleton-error { position: relative; background: linear-gradient(90deg, rgba(255,255,255,0.02), rgba(255,255,255,0.08), rgba(255,255,255,0.02)); }
.skeleton-error-icon{ position:absolute; right:8px; top:8px; background: rgba(239,68,68,0.12); color:#f87171; border-radius:50%; width:26px; height:26px; display:flex; align-items:center; justify-content:center; font-weight:700 }

@media (max-width: 560px){
  .barcode-modal{ min-width: auto; width: 96vw; max-height: 88vh; }
  .barcode-info{ grid-template-columns: 1fr; }
  .barcode-modal-actions{ flex-wrap: wrap; gap: 8px; }
  .qr-preview{ width: 240px; height: 240px; }
}
</style>
