<template>
  <div class="inactivity-settings-container">
    ...
    <div v-else class="wizard-container">
      ...
      <!-- Step 2: Tiempo de Inactividad -->
      <div v-if="currentStep === 1 && inactivityEnabled" class="step-content">
        <div class="step-header">
          <h2>{{ steps[1].title }}</h2>
          <p class="step-subtitle">¿Cuánto tiempo sin actividad antes de cerrar sesión?</p>
        </div>
        ...
        <div class="preset-grid">
          <button
            ...
            @click="inactivityMinutes = preset.value"
          >
        ...
        <div class="info-box">
          <strong>Seleccionaste:</strong> {{ inactivityMinutes }} minuto(s) de inactividad
        </div>
      </div>
      ...
      <div v-if="currentStep === 2" class="step-content">
        ...
        <div class="summary-box">
          ...
          <div v-if="inactivityEnabled" class="summary-item">
            ...
            <span class="summary-value">{{ inactivityMinutes }} minuto(s)</span>
          </div>
        ...
        <div v-if="inactivityEnabled" class="how-it-works">
          <h4>Cómo funciona:</h4>
          <ul>
            <li>Si no haces clic, tecleas o te desplazas durante {{ inactivityMinutes }} minutos...</li>
...
const presets = [
  { label: '1 min', value: 1, icon: '⚡⚡', description: 'Extremadamente corto (zonas muy públicas)' },
  { label: '5 min', value: 5, icon: '⚡', description: 'Muy corto (para zonas públicas)' },
  { label: '10 min', value: 10, icon: '⏱️', description: 'Corto' },
  { label: '30 min', value: 30, icon: '⏰', description: 'Recomendado (balance seguridad/comodidad)' },
  { label: '1 hora', value: 60, icon: '🕐', description: 'Largo' },
  { label: '2 horas', value: 120, icon: '🕑', description: 'Muy largo' }
]
...
const saveConfiguration = async () => {
  try {
    ...
    if (inactivityMinutes.value < 1 || inactivityMinutes.value > 480) {
      saveStatus.value = {
        type: 'error',
        message: '❌ Tiempo inválido. Debe estar entre 1 y 480 minutos.'
      }
      return
    }
