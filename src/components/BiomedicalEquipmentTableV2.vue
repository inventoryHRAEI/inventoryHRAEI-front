  ...
  <div class="biomedical-table-container" :class="containerThemeClass">
          ...
          <div class="modal-container glass-panel">
            <div class="modal-header">
              <div class="modal-title-section">
                ...
                <div>
                  <h2 class="modal-title">{{ getMainLabel(selectedEquipment) }}</h2>
            ...
            <div class="modal-body" v-if="selectedEquipment">
              ...
              <div class="quick-info-grid">
                <div class="quick-actions">
                  <button v-if="showMaintenanceActions" @click="handleRequestMaintenance(selectedEquipment)" class="modal-action-btn maint-btn">
    ...
    <div class="table-container glass-panel">
      ...
      <div v-if="loading" class="loading-state">
        ...
        <p>Cargando {{ props.entityLabel }}...</p>
      </div>
      ...
      <!-- Empty State -->
        ...
        <p v-if="searchTerm || activeFiltersCount > 0">
          No se encontraron {{ props.entityLabel }} con los filtros actuales
        </p>
        <p v-else>No hay {{ props.entityLabel }} disponibles</p>
        <button 
        ...
        <div class="table-container-inner">
          ...
          <thead>
            <tr class="header-row">
              ...
              <!-- Actions Column -->
              <th class="actions-col sticky-col-right" :title="actionListTitle">
                <i class="pi pi-cog"></i>
              </th>
            </tr>
          </thead>
            ...
            >
              ...
              <td class="actions-col sticky-col-right" @click.stop>
                <div class="action-buttons">
                  <button 
                    v-if="allowedActionsSet.has('view') || allowedActionsSet.has('kardex')"
                    @click.stop="handleViewAction(item)" 
                    class="action-btn view-btn"
                    :title="allowedActionsSet.has('kardex') ? 'Ver Kardex' : 'Ver detalles'"
                  >
                    <i class="pi pi-eye"></i>
                  </button>
                  <button 
                    v-if="allowedActionsSet.has('edit')"
                  ...
                  <div class="action-menu">
                    ...
                    <Transition name="menu-pop">
                      <div v-if="openMenuIdx === idx" class="action-dropdown">
                        <button v-if="allowedActionsSet.has('barcode')" @click="handleShowBarcode(item)" class="menu-item">
                          <i class="pi pi-qrcode"></i> Código QR
                        </button>
                        <button v-if="allowedActionsSet.has('maintenance')" @click="handleRequestMaintenance(item)" class="menu-item">
                          <i class="pi pi-wrench"></i> Mantenimiento
                        </button>
                      </div>
                    </Transition>
    ...
    <div class="pagination-footer glass-panel">
      <div class="pagination-info">
        <span class="info-badge">
          ...
          {{ filteredData.length }} {{ props.entityLabel }}
        </span>
    ...
    <ColumnFilterPanel
      ...
      :anchor-el="filterAnchorEl"
        ...
        @select-filtered-rows="handleSelectFilteredRows"
    />
  </div>
</template>
...
const props = defineProps({
  ...
  // Controla qué pasa al hacer clic en la fila completa.
  // Valores posibles:
  // - 'auto' (default): comportamiento actual (abre detalle si 'view' está permitido)
  // - 'none': deshabilita la acción de clic en fila (solo botones explícitos disparan acciones)
  rowClickAction: { type: String, default: 'auto' },
  showMaintenanceActions: { type: Boolean, default: true },
  entityLabel: { type: String, default: 'equipos biomédicos' },
  allowedActions: { type: Array, default: () => ['view', 'edit', 'kardex', 'barcode', 'maintenance'] }
})

const emit = defineEmits(['view-item', 'edit-item', 'show-barcode', 'refresh', 'request-maintenance', 'action-not-allowed'])

const showMaintenanceActions = computed(() => props.showMaintenanceActions)
const allowedActionsSet = computed(() => new Set(Array.isArray(props.allowedActions) ? props.allowedActions : []))
...
const actionListTitle = computed(() => {
  const list = Array.isArray(props.allowedActions) ? props.allowedActions : []
  return list.map(a => actionLabels[a] || a).join(', ')
})
...
const allColumns = computed(() => {
  if (props.showAllColumns && props.data.length > 0) {
    // Build a union of all keys across the dataset so we don't miss columns
    const seen = new Set()
    const ordered = []

    // Prefer keys order from the first row, then append any new keys found in other rows
    const first = props.data[0]
    if (first && typeof first === 'object') {
      Object.keys(first).forEach(k => {
        if (!seen.has(k) && k !== '__hasRealData') {
          seen.add(k)
          ordered.push(k)
        }
      })
    ...
    // Evitar duplicados semánticos comunes provenientes de alias normalizados
    // (ej. backend devuelve "Clave  HRAEI" y además "claveHRAEI", o
    // "Descripción del bien" y además "descripcion").
    const removeKeys = new Set()

    const hasHumanClave = ordered.some(k => ['Clave  HRAEI', 'Clave HRAEI', 'CLAVE HRAEI'].includes(k))
    if (hasHumanClave) {
      ;['claveHRAEI', 'clave_hraei', 'claveHraei'].forEach(k => {
        if (ordered.includes(k)) removeKeys.add(k)
      })
    }

    const hasHumanDescripcion = ordered.some(k => [
    ...
    if (hasHumanDescripcion) {
      ;['descripcion', 'descripcionDelBien'].forEach(k => {
        if (ordered.includes(k)) removeKeys.add(k)
      })
    }

    const finalKeys = ordered.filter(k => !removeKeys.has(k))
    return finalKeys.map(key => ({ field: key, header: formatHeader(key) }))
  }
...
function applyGlobalSearch(source) {
  if (!searchTerm.value) return [...source]
  const search = normalizeFilterValue(searchTerm.value)
  return source.filter(item => Object.values(item).some(val => normalizeFilterValue(val).includes(search)))
}

function applyColumnFilters(source, excludeField = null) {
  let result = [...source]
  Object.keys(columnFilters.value).forEach(field => {
    ...
    const selectedNormalized = new Set(filterValues.map(v => normalizeFilterValue(v)))
    result = result.filter(item => selectedNormalized.has(normalizeFilterValue(item[field])))
  })
  return result
}
...
const filteredData = computed(() => {
  let result = applyGlobalSearch(props.data)
...
function getUniqueValues(field, sourceData = props.data) {
  const values = new Set()
  ;(Array.isArray(sourceData) ? sourceData : []).forEach(item => {
    values.add(String(item[field] ?? ''))
  })
  return Array.from(values).sort((a, b) => a.localeCompare(b, 'es', { numeric: true }))
}

function getBaseDataForColumnFilter(field) {
  const withSearch = applyGlobalSearch(props.data)
  return applyColumnFilters(withSearch, field)
}

function getFilteredUniqueValues(field) {
  const values = getUniqueValues(field, getBaseDataForColumnFilter(field))
  const search = (columnFilterSearch.value[field] || '').toLowerCase()
  if (!search) return values
  return values.filter(v => v.toLowerCase().includes(search))
}

function getValueCount(field, value, sourceData = props.data) {
  const normalizedValue = normalizeFilterValue(value)
  return (Array.isArray(sourceData) ? sourceData : []).filter(item => normalizeFilterValue(item[field]) === normalizedValue).length
}

function hasActiveFilter(field) {
  const filter = columnFilters.value[field]
  if (!Array.isArray(filter)) return false
  const totalAvailable = getUniqueValues(field, getBaseDataForColumnFilter(field)).length
  return filter.length !== totalAvailable
}
...
function selectAllFilterValues(field) {
  columnFilters.value[field] = [...getUniqueValues(field, getBaseDataForColumnFilter(field))]
  currentPage.value = 1
}
...
function openColumnFilter(event, col) {
  ...
  const availableValues = getUniqueValues(col.field, getBaseDataForColumnFilter(col.field))

  if (!Array.isArray(columnFilters.value[col.field])) {
    columnFilters.value[col.field] = [...availableValues]
  } else if (columnFilters.value[col.field].length > 0) {
    // Mantener solo valores válidos para el rango actualmente filtrado
    columnFilters.value[col.field] = columnFilters.value[col.field].filter(v => availableValues.includes(v))
  }
...
// Seleccionar todas las filas que cumplen los filtros actuales (cross-page)
function handleSelectFilteredRows() {
  // Mapeo clave->índice global para props.data
  const keyToIndex = new Map()
  props.data.forEach((item, i) => {
    const key = String(getItemKey(item, i) ?? i)
    keyToIndex.set(key, i)
  })

  // Obtener índices de los items actualmente filtrados
  const indices = []
  ...
  // Llevar la paginación a la primera página con selección (opcional)
  if (unique.length > 0) {
    const first = unique[0]
    const page = Math.floor(first / internalPageSize.value) + 1
...
const activeColumnFilterItems = computed(() => {
  ...
  const baseData = getBaseDataForColumnFilter(field)
  const values = getUniqueValues(field, baseData)
  return values.map(v => ({
    value: v,
    label: v || '(Vacio)',
    count: getValueCount(field, v, baseData)
  }))
})
...
function initializeColumnVisibility() {
  ...
  allColumns.value.forEach(col => {
    // Modo agresivo: si showAllColumns está activo, TODAS visibles por defecto.
    // (Sin excepciones para evitar arranque con columnas ocultas.)
...
// Toggle column selector and ensure visibility map includes all known columns
function toggleColumnSelector() {
  showColumnSelector.value = !showColumnSelector.value
  if (showColumnSelector.value) {
    // ensure every column from allColumns exists in the visibility map
    const vis = { ...columnVisibility.value }
    allColumns.value.forEach(col => {
      if (vis[col.field] === undefined) vis[col.field] = props.showAllColumns ? true : (priorityColumns.includes(col.field))
    })

    // En modo showAllColumns, abrir selector debe reflejar TODAS palomeadas.
    if (props.showAllColumns) {
      allColumns.value.forEach(col => {
        vis[col.field] = true
      })
...
function onRowClick(item) {
  // Si el modo de clic en fila está desactivado, no hacer nada.
  if (props.rowClickAction === 'none') return

  if (allowedActionsSet.value.has('view')) {
    openDetailModal(item)
  }
}

function onRowDblClick(item) {
  if (allowedActionsSet.value.has('edit')) {
    handleEditItem(item)
  }
}

function handleViewAction(item) {
  // Prefer kardex emit when available (parent handles preview/kardex)
  if (allowedActionsSet.value.has('kardex')) {
    emit('view-item', item)
    return
  }

  // Fallback to internal detail view when 'view' is allowed
  if (allowedActionsSet.value.has('view')) {
    openDetailModal(item)
    return
  }

  // Otherwise notify parent that the action is not allowed
  emit('action-not-allowed', { action: 'view', item })
}
...
function getMainLabel(item) {
  if (!item) return props.entityLabel || 'Artículo'
  const candidates = [
    item.EQUIPO_MEDICO,
    item.DESCRIPCION,
  ...
  return props.entityLabel || 'Artículo'
}
...
// Re-initialize visibility whenever the computed set of all columns changes
watch(allColumns, (cols) => {
  // If showAllColumns is active, ensure newly discovered columns are marked visible
  if (props.showAllColumns) {
    const vis = { ...columnVisibility.value }
    cols.forEach(col => {
      // Modo agresivo: forzar true para todas cuando showAllColumns está activo
      vis[col.field] = true
    })
    columnVisibility.value = vis
  } else {
    // If not showing all, still ensure new columns get default visibility (priority or false)
    initializeColumnVisibility()
  }
}, { immediate: true })
...
.table-container {
  ...
  min-height: 350px;
  position: relative;
  /* Permitir scroll horizontal y vertical para tablas anchas */
  overflow-x: auto;
  overflow-y: auto;
  max-height: 600px;
...
.table-container {
  max-height: 600px;
  overflow-y: auto;
  overflow-x: auto; /* permitir scroll horizontal si las columnas no caben */
  scrollbar-gutter: stable;
}
...
.data-cell {
  ...
  max-width: none; /* permitir que el contenido ocupe el espacio necesario */
}
