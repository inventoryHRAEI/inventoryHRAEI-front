<template>
    <div class="smart-search-wrapper" ref="wrapperRef">
        ...
        <div class="smart-search-input-box">
            <input ref="inputRef" class="smart-input" :class="{ 'has-na': modelValue === 'N/A' }" type="text"
                :value="modelValue" :placeholder="placeholder" @input="handleInput" @focus="onFocus" @blur="onBlur"
                            ...
                            <div v-for="(result, ri) in group.items" :key="`${group.key}-${ri}`"
                                ...
                                <div class="card-details">
                                    <span v-if="hasVal(result.item.marca)" class="detail-chip chip-marca">
                                        <span class="chip-label">Marca</span> <span
                                            v-html="highlightField(result, 'marca')"></span>
                                    </span>
                                    <span v-if="hasVal(result.item.modelo)" class="detail-chip chip-modelo">
                                        <span class="chip-label">Modelo</span> <span
                                            v-html="highlightField(result, 'modelo')"></span>
                                    </span>
                                    <span v-if="hasVal(result.item.serie)" class="detail-chip chip-serie">
                                        <span class="chip-label">Serie</span> <span
                                            v-html="highlightField(result, 'serie')"></span>
                                    </span>
                                    <span v-if="hasVal(result.item.noInventario)" class="detail-chip chip-inv">
                                        <span class="chip-label">Inv</span> <span
                                            v-html="highlightField(result, 'noInventario')"></span>
                                    </span>
                                    <span v-if="hasVal(result.item.referencia)" class="detail-chip chip-ref">
                                        <span class="chip-label">Ref</span> <span
                                            v-html="highlightField(result, 'referencia')"></span>
                                    </span>
                                    <span v-if="hasVal(result.item.claveHRAEI)" class="detail-chip chip-clave">
                                        <span class="chip-label">HRAEI</span> <span
                                            v-html="highlightField(result, 'claveHRAEI')"></span>
                                    </span>
                                    <span v-if="hasVal(result.item.ubicacion) && result.item.ubicacion !== 'N/A'"
                                        class="detail-chip chip-ubi">
                                        <span class="chip-label">📍</span> <span
                                            v-html="highlightField(result, 'ubicacion')"></span>
                                    </span>
                                </div>
                            </div>
                        </template>
