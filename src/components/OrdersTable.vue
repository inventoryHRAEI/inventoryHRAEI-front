    ...
    <div class="orders-table-container">
        ...
        <div v-if="paginatedOrders.length > 0" class="multi-delete-toolbar">
            <div class="toolbar-left">
                ...
                <label v-if="hasGlobalDeletePermission" class="toggle-switch">
            ...
            <button v-if="multiDeleteMode && selectedForDelete.size > 0 && hasGlobalDeletePermission" @click="deleteSelected"
        ...
        <div class="table-surface">
            ...
            <table class="orders-table">
                <thead>
                    <tr>
                        <th v-if="multiDeleteMode && hasGlobalDeletePermission" class="checkbox-col">
                ...
                <tbody>
                    <template v-for="order in paginatedOrders" :key="order.id">
                        ...
                        <tr class="table-row"
                            ...
                            <td v-if="multiDeleteMode && hasGlobalDeletePermission" class="checkbox-cell" data-label="Seleccionar">
                                    ...
                                    <template v-else-if="col.key === 'acciones'">
                                        <template v-if="isOrderActive(order)">
                                            ...
                                            <button @click="$emit('upload', order)" class="action-btn upload-btn" title="Subir archivos">
                                             ...
                                             <button v-if="canEditOrderFor(order)" @click="$emit('edit', order)" class="action-btn edit-btn" title="Editar">
                                                 <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                                                     <path d="M17 3a2.828 2.828 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5L17 3z"></path>
                                                 </svg>
                                             </button>
                                             <button v-if="!canEditOrderFor(order) && onRequestEdit" @click="onRequestEdit(order)" class="action-btn request-btn" title="Solicitar permiso de edición">
                                                 <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                                                     <circle cx="12" cy="12" r="10"></circle>
                                                     <line x1="12" y1="8" x2="12" y2="12"></line>
                                            ...
                                            <button v-if="hasGlobalDeletePermission" @click="$emit('delete', order.id)" class="action-btn delete-btn" title="Cancelar orden">
                                                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                                                    <polyline points="3 6 5 6 21 6"></polyline>
...
const props = defineProps({
    ...
    // Permisos base
    canEditOrder: { type: Boolean, default: false },
    canEditSpecificOrder: { type: Function, default: null },
    canDeleteOrder: { type: Boolean, default: false },
    // Función para solicitar edición
    onRequestEdit: { type: Function, default: null }
})

const emit = defineEmits(['edit', 'excel', 'delete', 'create', 'deleteMultiple', 'openHistory', 'upload', 'generateSalida', 'generateEntrada', 'requestEdit'])
...
const hasGlobalEditPermission = computed(() => {
    return Boolean(props.canEditOrder?.value ?? props.canEditOrder)
})

const hasGlobalDeletePermission = computed(() => {
    return Boolean(props.canDeleteOrder?.value ?? props.canDeleteOrder)
})

function canEditOrderFor(order) {
    if (hasGlobalEditPermission.value) return true
    if (typeof props.canEditSpecificOrder === 'function') {
        try {
            return Boolean(props.canEditSpecificOrder(order))
        } catch (error) {
            console.warn('canEditSpecificOrder threw an error', error)
        }
    }
    return false
}
...
function getColSpan() {
    const selection = (multiDeleteMode.value && hasGlobalDeletePermission.value) ? 1 : 0
