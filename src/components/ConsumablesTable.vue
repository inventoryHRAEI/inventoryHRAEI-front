<template>
    <div class="consumables-table-wrapper">
        ...
        <div class="table-controls">
            <div class="controls-left">
                ...
                <div class="expiration-filter">
                    <label class="expiration-label">Caducidad:</label>
        ...
        <div class="table-scroll-container" ref="tableScrollRef">
            ...
            <!-- Usar componente compartido ColumnFilterPanel (estilos + posicionamiento) -->
            <ColumnFilterPanel
                :isOpen="!!activeColumnFilter"
                :title="activeColumnFilter"
                :items="buildFilterItems(activeColumnFilter)"
                :selected="(activeFilters[activeColumnFilter]) || []"
                :anchorEl="activeFilterAnchor"
                @update:selected="onPanelUpdateSelected"
                @apply="onPanelApply"
                @close="onPanelClose"
                @select-filtered-rows="onPanelSelectFilteredRows"
            />
...
// Funciones de selección (basadas en IDs estables por item)
function getSelectedSet() {
    return new Set((selectedRows.value || []).map(v => String(v)))
}

const allSelected = computed(() => {
    if (paginatedItems.value.length === 0) return false
    const set = getSelectedSet()
    return paginatedItems.value.every(item => set.has(String(getItemKey(item))))
})
...
function isRowSelected(item) {
    const id = String(getItemKey(item))
    return getSelectedSet().has(id)
}

function toggleRowSelection(item) {
    const id = String(getItemKey(item))
    const arr = Array.isArray(selectedRows.value) ? [...selectedRows.value] : []
...
function toggleSelectAll() {
    const pageIds = paginatedItems.value.map(item => String(getItemKey(item)))
    if (allSelected.value) {
        // Deseleccionar todos en esta página
        selectedRows.value = (selectedRows.value || []).filter(id => !pageIds.includes(String(id)))
    } else {
        // Añadir todos en esta página a la selección (append)
        const set = getSelectedSet()
        pageIds.forEach(id => set.add(String(id)))
...
async function exportSelectedToExcel() {
  ...
  try {
    ...
    // Obtener los datos seleccionados usando IDs estables
    const selectedSet = new Set((selectedRows.value || []).map(v => String(v)))
    const selectedData = props.items.filter(item => selectedSet.has(String(getItemKey(item))))
...
const filterSearchDebounce = ref({}) // Debounce timers for filter search

// Dropdown position via getBoundingClientRect
const dropdownStyle = ref({})
const tableScrollRef = ref(null)
// Anchor element for the shared ColumnFilterPanel (DOM element)
...
function toggleColumnFilter(col, event) {
    ...
    if (btn) {
        ...
        // almacenar ancla para el ColumnFilterPanel
        activeFilterAnchor.value = btn
    }
    activeColumnFilter.value = col
    // Initialize filter search if not exists
    if (!columnFilterSearch.value[col]) {
        columnFilterSearch.value = { ...columnFilterSearch.value, [col]: '' }
    }
    // Initialize filter with all values if not exists
...
// Debounced filter search handler
const debouncedFilterSearch = debounce((col, value) => {
    columnFilterSearch.value = { ...columnFilterSearch.value, [col]: value }
}, 150)

function handleFilterSearchInput(col, event) {
    debouncedFilterSearch(col, event.target.value)
}
...
function getFilteredUniqueValues(col) {
    ...
    const searchSet = new Set(vals.filter(v => String(v).toLowerCase().includes(search)))
    return vals.filter(v => searchSet.has(v))
}
...
function toggleFilterValue(col, val) {
    ...
    const current = activeFilters.value[col] ? [...activeFilters.value[col]] : [...allVals]
...
function selectAllFilteredResults(col) {
    // Añadir a la selección actual (append) todos los items cuyo valor en la columna 'col'
    // coincide con alguno de los valores seleccionados en el filtro de esa columna.
    const current = activeFilters.value[col]
    if (!current || !Array.isArray(current) || current.length === 0) return

    const valueSet = new Set(current.map(v => String(v)))
    const merged = getSelectedSet() // Set con selección actual

    // Try to use prebuilt index for fast selection
    const index = uniqueValuesIndexCache.value[col] || uniqueValuesIndexPartial.value[col]
    if (index) {
        for (const v of valueSet) {
            const ids = index[String(v)] || []
            ids.forEach(id => merged.add(String(id)))
        }
    } else {
        // Fallback to full scan if index not ready
        props.items.forEach(item => {
            const cellVal = getCellRawValue(item, col, '')
            const normalized = cellVal !== undefined && cellVal !== null ? String(cellVal).trim() : ''
            if (valueSet.has(normalized)) merged.add(String(getItemKey(item)))
        })
    }
...
function selectFilteredRowsByValues(col, values) {
    if (!col || !Array.isArray(values) || values.length === 0) return
    const valueSet = new Set(values.map(v => String(v)))
    const merged = getSelectedSet()
    const index = uniqueValuesIndexCache.value[col] || uniqueValuesIndexPartial.value[col]
    if (index) {
        for (const v of valueSet) {
            const ids = index[String(v)] || []
...
function buildFilterItems(col) {
    if (!col) return []
    const arr = (uniqueValuesCache.value[col] || uniqueValuesPartial.value[col] || [])
    const counts = uniqueValuesCountCache.value[col] || uniqueValuesCountPartial.value[col] || {}
    return (arr || []).map(v => ({ value: v, label: v, count: counts[v] || 0 }))
}

function onPanelUpdateSelected(vals) {
    if (!activeColumnFilter.value) return
    activeFilters.value = { ...activeFilters.value, [activeColumnFilter.value]: vals }
}

function onPanelApply() {
    if (!activeColumnFilter.value) return
    applyAndCloseFilter(activeColumnFilter.value)
}
...
function onPanelSelectFilteredRows(vals) {
    if (!activeColumnFilter.value) return
    selectFilteredRowsByValues(activeColumnFilter.value, vals)
}
...
function getFilterCount(col) {
    const filter = activeFilters.value[col]
    if (!filter || !Array.isArray(filter)) return 0
    const allVals = getUniqueValuesFor(col)
    // Return count of excluded values (values NOT in filter)
...
function applyAndCloseFilter(col) {
    const allVals = getUniqueValuesFor(col)
    const current = activeFilters.value[col]
    // If filter is empty or all values selected, remove it
    if (!current || !Array.isArray(current) || current.length === 0 || current.length >= allVals.length) {
...
// Index mapping value -> [itemId,...] per column for fast selection (built in background)
const uniqueValuesIndexCache = ref({})
const uniqueValuesIndexPartial = ref({})

// Smaller synchronous sample for immediate UI and chunked full scan
const SAMPLE_SCAN = 200 // Increased for better initial representation
const MAX_DISPLAY_VALUES = 300 // Reduced to keep UI responsive
const MAX_FILTER_RENDER = 100 // Reduced to prevent DOM bloat
...
function scheduleComputeUniqueValues(col) {
    ...
    const BATCH_SIZE = 800 // Increased for faster processing
    const len = props.items.length
    let idx = 0
    const set = new Set()
    const counts = {}
    const indexMap = {} // value -> array of item IDs (partial index built incrementally)
    ...
    const sortAndStorePartial = () => {
        ...
        // expose a shallow snapshot of the partial index so UI actions can use it
        uniqueValuesIndexPartial.value = { ...uniqueValuesIndexPartial.value, [col]: { ...indexMap } }
    }
    ...
    const finalize = () => {
        ...
        uniqueValuesIndexPartial.value = { ...uniqueValuesIndexPartial.value, [col]: undefined }
    }
        ...
        try {
            ...
            for (; idx < end; idx++) {
                ...
                if (value !== undefined && value !== null && String(value).trim() !== '') {
                    ...
                    try {
                        const id = String(getItemKey(item))
                        if (!indexMap[normalized]) indexMap[normalized] = []
                        indexMap[normalized].push(id)
                    } catch (err) {
            ...
            if (idx < len && set.size < MAX_DISPLAY_VALUES) {
                ...
                if (typeof window !== 'undefined' && window.requestIdleCallback) {
                    window.requestIdleCallback(processChunk, { timeout: 100 }) // Reduced timeout
                } else {
                    setTimeout(processChunk, 20) // Reduced timeout
                }
            } else {
                // store final index and finish
                uniqueValuesIndexCache.value = { ...uniqueValuesIndexCache.value, [col]: indexMap }
                finalize()
            }
        } catch (e) {
    ...
    // Kick off immediately for better responsiveness
    if (typeof window !== 'undefined' && window.requestIdleCallback) {
        window.requestIdleCallback(processChunk, { timeout: 50 }) // Reduced timeout
    } else {
        setTimeout(processChunk, 10) // Reduced timeout
    }
}
...
function getValueCount(col, val) {
    ...
    const index = uniqueValuesIndexCache.value[col] || uniqueValuesIndexPartial.value[col]
    if (index && Array.isArray(index[key])) return index[key].length

    const counts = uniqueValuesCountCache.value[col] || uniqueValuesCountPartial.value[col]
    if (counts && typeof counts[key] === 'number') return counts[key]

    // fallback (rare) — compute on demand with early exit
    let count = 0
    for (const item of props.items) {
        const itemVal = String(getCellRawValue(item, col, '')).trim()
        if (itemVal === key) {
            count++
            // Early exit if we found enough for display
            if (count > 1000) break
        }
    }
    return count
...
function isColumnFiltered(columnName) {
    ...
    if (!filter || !Array.isArray(filter)) return false
    const allVals = getUniqueValuesFor(columnName)
    // Filter is active if it has values AND not all values are selected
    return filter.length > 0 && filter.length < allVals.length
}
...
const filterDropdownValues = computed(() => {
    ...
    if (!search) {
        // No search filter, return limited set
        return all.length > MAX_FILTER_RENDER ? all.slice(0, MAX_FILTER_RENDER) : all
    }
    // Apply search filter with Set for O(1) lookup
    const searchLower = search.toLowerCase()
    const filtered = all.filter(v => String(v).toLowerCase().includes(searchLower))
    return filtered.length > MAX_FILTER_RENDER ? filtered.slice(0, MAX_FILTER_RENDER) : filtered
})
...
const filterDropdownTotalCount = computed(() => {
    ...
    const search = (columnFilterSearch.value[col] || '').toLowerCase()
    const all = getUniqueValuesFor(col)
    if (!search) return all.length
    const searchLower = search.toLowerCase()
    return all.filter(v => String(v).toLowerCase().includes(searchLower)).length
})
...
watch(
    () => props.items,
    (newItems, oldItems) => {
        // Only clear if items actually changed
        if (newItems.length !== oldItems?.length) {
            ...
            uniqueValuesIndexCache.value = {}
            uniqueValuesIndexPartial.value = {}
            activeFilters.value = {}
            columnFilterSearch.value = {}
            currentPage.value = 1
        }
    },
    { deep: false } // Changed to shallow watch for better performance
)
...
const realActiveFilters = computed(() => {
    ...
    for (const [col, vals] of Object.entries(activeFilters.value)) {
        ...
        const allVals = getUniqueValuesFor(col)
        // Only include filter if   it's a proper subset of all values
...
// Cache of normalized keys per item to avoid rebuilding lookup maps repeatedly
const __normalizedKeyCache = new WeakMap()
...
function pickValue(item, keys = [], fallback = '') {
    ...
    let normalizedLookup = __normalizedKeyCache.get(item)
    if (!normalizedLookup) {
        ...
        __normalizedKeyCache.set(item, normalizedLookup)
    }
...
const handleScrollForFilter = () => {
    ...
    const targetBtn = tableEl.querySelector(`th[data-column="${activeColumnFilter.value}"] .col-filter-btn`)
