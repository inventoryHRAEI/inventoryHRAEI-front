                ...
                <div class="wizard-modal">
                    ...
                    <div v-if="flowStatus.in_progress" class="flow-status-banner in-progress">
                        ...
                        <div class="banner-content">
                            ...
                            <p>Tipo: <strong>{{ getMaintenanceTypeLabel(flowStatus.in_progress.maintenance_type)
                                    }}</strong></p>
                    ...
                    <div class="wizard-body">
                        ...
                        <div v-if="flowStatus.in_progress" class="step-content">
                            ...
                            <div class="form-group">
                                ...
                                <div class="status-options">
                                    <label
                                        :class="['status-option', { selected: finishForm.result_status === 'functional' }]">
                                    ...
                                    <label
                                        :class="['status-option', { selected: finishForm.result_status === 'non_functional' }]">
                            ...
                            <div class="form-group">
                                ...
                                <textarea v-model="finishForm.diagnostico" class="form-textarea"
                                    placeholder="Describe el diagnóstico realizado..." rows="3"></textarea>
                            </div>

                            <div class="form-group">
                                <label>Causa de Falla (si aplica)</label>
                                <textarea v-model="finishForm.causa" class="form-textarea"
                                    placeholder="Describe la causa de la falla..." rows="2"></textarea>
                            </div>

                            <div class="form-group">
                                <label>Observaciones / Trabajo Realizado</label>
                                <textarea v-model="finishForm.observaciones" class="form-textarea"
                                    placeholder="Describe las observaciones, refacciones usadas, etc."
                                    rows="4"></textarea>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" v-model="finishForm.finished_by" class="form-input"
                                    placeholder="Nombre de quien termina el mantenimiento" />
                            </div>
                            ...
                            <div class="finish-summary">
                                ...
                                <div class="summary-grid">
                                    ...
                                    <div class="summary-item">
                                        ...
                                        <span
                                            :class="['summary-value', finishForm.result_status === 'functional' ? 'functional' : 'non-functional']">
                        ...
                        <div v-else-if="currentStep === 0" class="step-content">
                            ...
                            <div class="maintenance-types">
                                <label v-for="type in maintenanceTypes" :key="type.value"
                                    ...
                                    <input type="radio" :value="type.value" v-model="form.maintenance_type"
                                        class="hidden-input" />
                            ...
                            <h4 class="step-title">¿Interno o Externo?</h4>
                            <div class="form-group">
                                <label>Tipo de Mantenimiento</label>
                                <div class="provider-options">
                                    <label
                                        :class="['provider-option', { selected: form.provider_type === 'internal' }]">
                                        <input type="radio" value="internal" v-model="form.provider_type"
                                            class="hidden-input" />
                                        <VueIcon name="fa-building" class="provider-icon" />
                                        <span>Interno</span>
                                    </label>
                                    <label
                                        :class="['provider-option', { selected: form.provider_type === 'external' }]">
                                        <input type="radio" value="external" v-model="form.provider_type"
                                            class="hidden-input" />
                                        <VueIcon name="fa-industry" class="provider-icon" />
                                        <span>Externo</span>
                                    </label>
                                </div>
                            </div>
                            ...
                            <div class="form-group">
                                <label>Responsable del Mantenimiento <span class="required">*</span></label>
                                <input type="text" v-model="form.maintenance_responsible" class="form-input"
                                    placeholder="Nombre de la persona responsable" required />
                                <span v-if="!form.maintenance_responsible" class="form-error">Este campo es
                                    obligatorio</span>
                            </div>

                            <div v-if="form.provider_type === 'external'" class="form-group">
                                <label>Empresa Proveedora <span class="required">*</span></label>
                                <input type="text" v-model="form.provider_company" class="form-input"
                                    placeholder="Nombre de la empresa de mantenimiento externo" required />
                                <span v-if="!form.provider_company" class="form-error">Este campo es obligatorio para
                                    mantenimiento externo</span>
                            </div>

                            <div v-if="form.provider_type === 'internal'" class="form-group">
                                <label>Departamento / Área del Hospital</label>
                                <input type="text" v-model="form.internal_department" class="form-input"
                                    placeholder="Ej: Biomédica, Ingeniería, Mantenimiento" />
                            </div>
                            ...
                            <div class="form-group">
                                <label>¿El equipo cuenta con garantía? <span class="required">*</span></label>
                                <div class="warranty-options">
                                    <label :class="['warranty-option', { selected: form.has_warranty === true }]">
                                        <input type="radio" :value="true" v-model.lazy="form.has_warranty"
                                            class="hidden-input" />
                                        <VueIcon name="bi-check-circle-fill" class="warranty-icon yes" />
                                        <span>Sí</span>
                                    </label>
                                    <label :class="['warranty-option', { selected: form.has_warranty === false }]">
                                        <input type="radio" :value="false" v-model.lazy="form.has_warranty"
                                            class="hidden-input" />
                            ...
                            <div v-if="form.has_warranty === true" class="form-group">
                                <label>Fecha de fin de garantía <span class="required">*</span></label>
                                <input type="date" v-model="form.warranty_end_date" class="form-input"
                                    placeholder="Fecha en la que vence la garantía" />
                                <span v-if="form.warranty_end_date && new Date(form.warranty_end_date) < new Date()"
                                    class="form-warning">⚠️ La garantía ya ha vencido</span>
                            </div>

                            <div class="form-group">
                                <label>¿Se encuentra en contrato de mantenimiento? <span
                                        class="required">*</span></label>
                                <div class="contract-options">
                                    <label
                                        :class="['contract-option', { selected: form.maintenance_contract === true }]">
                                        <input type="radio" :value="true" v-model.lazy="form.maintenance_contract"
                                            class="hidden-input" />
                                        <VueIcon name="bi-check-circle-fill" class="contract-icon preventive" />
                                        <span class="contract-label">Preventivo</span>
                                        <span class="contract-desc">Revisiones periódicas</span>
                                    </label>
                                    <label
                                        :class="['contract-option', { selected: form.maintenance_contract === false }]">
                                        <input type="radio" :value="false" v-model.lazy="form.maintenance_contract"
                                            class="hidden-input" />
                                        <VueIcon name="bi-x-circle-fill" class="contract-icon corrective" />
                                        <span class="contract-label">Correctivo</span>
                                        <span class="contract-desc">Solo reparaciones</span>
                                    </label>
                                    <label
                                        :class="['contract-option', { selected: form.maintenance_contract === null }]">
                                        <input type="radio" :value="null" v-model.lazy="form.maintenance_contract"
                                            class="hidden-input" />
                        ...
                        <div v-if="currentStep === 1" class="step-content">
                            <h4 class="step-title">Ubicación de Retorno</h4>

                            <div class="form-group">
                                <label>Ubicación de retorno / área destino</label>
                                <input type="text" v-model="form.return_location" class="form-input"
                                    placeholder="Ej: Biomedica, Emergencias" />
                            </div>
                        </div>
                        ...
                        <div v-if="currentStep === 2" class="step-content">
                            ...
                            <div class="form-group">
                                <label>Estado Final del Equipo</label>
                                <div class="status-options">
                                    <label :class="['status-option', { selected: form.status === 'functional' }]">
                                        <input type="radio" value="functional" v-model="form.status"
                                            class="hidden-input" />
                                        <VueIcon name="bi-check-circle-fill" class="status-icon functional" />
                                        <span>Funcional</span>
                                    </label>
                                    <label :class="['status-option', { selected: form.status === 'non_functional' }]">
                                        <input type="radio" value="non_functional" v-model="form.status"
                                            class="hidden-input" />
                                        <VueIcon name="bi-x-circle-fill" class="status-icon non-functional" />
                                        <span>No Funcional</span>
                                    </label>
                            ...
                            <div class="form-group">
                                <label>Diagnóstico</label>
                                <textarea v-model="form.diagnostico" class="form-textarea"
                                    placeholder="Describe el diagnóstico realizado..." rows="3"></textarea>
                            </div>

                            <div class="form-group">
                                <label>Causa de Falla (si aplica)</label>
                                <textarea v-model="form.causa" class="form-textarea"
                            ...
                            <div class="maintenance-summary">
                                ...
                                <div class="summary-grid">
                                    <div class="summary-item">
                                        ...
                                        <span class="summary-value">{{ getMaintenanceTypeLabel(form.maintenance_type)
                                            }}</span>
                                    </div>
                                    <div class="summary-item">
                                    ...
                                    <div class="summary-item">
                                        ...
                                        <span class="summary-value">{{ form.maintenance_responsible || 'Sin especificar'
                                            }}</span>
                                    </div>
                                    <div class="summary-item">
                                        <span class="summary-label">{{ form.provider_type === 'external' ? 'Empresa' :
                                            'Departamento' }}:</span>
                                        <span class="summary-value">{{ form.provider_company || form.internal_department
                                            || 'Sin especificar' }}</span>
                                    </div>
                                    <div class="summary-item">
                                        <span class="summary-label">Tipo:</span>
                                        <span
                                            :class="['summary-value', form.provider_type === 'internal' ? 'internal' : 'external']">
                                            {{ form.provider_type === 'internal' ? '🏢 Interno' : '🏭 Externo' }}
                                        </span>
                                    </div>
                                    <div class="summary-item">
                                        <span class="summary-label">Estado:</span>
                                        <span
                                            :class="['summary-value', form.status === 'functional' ? 'functional' : 'non-functional']">
                                            {{ form.status === 'functional' ? 'Funcional' : 'No Funcional' }}
                                        </span>
                                    </div>
                                    <div class="summary-item">
                                        <span class="summary-label">Garantía:</span>
                                        <span
                                            :class="['summary-value', form.has_warranty ? 'warranty-yes' : 'warranty-no']">
                                            {{ form.has_warranty ? '✅ Sí' : '❌ No' }}{{ form.has_warranty ? ` (hasta
                                    ...
                                    <div class="summary-item">
                                        ...
                                        <span
                                            :class="['summary-value', form.maintenance_contract === true ? 'contract-preventive' : form.maintenance_contract === false ? 'contract-corrective' : 'contract-none']">
                                            {{ form.maintenance_contract === true ? '🛡️ Preventivo' :
                                                form.maintenance_contract === false ? '🔧 Correctivo' : '─ Ninguno' }}
                                        </span>
                                    </div>
                                </div>
                            </div>
                    ...
                    <footer class="wizard-footer">
                        ...
                        <template v-else>
                            ...
                            <button v-else @click="submitMaintenance" class="btn btn-success" :disabled="saving">
                                <span v-if="saving">Guardando...</span>
                                <span v-else>
...
const form = ref({
    ...
    maintenance_responsible: '', // responsable del mantenimiento
    provider_company: '',        // empresa del responsable (solo si es externo)
...
const canProceed = computed(() => {
    if (currentStep.value === 0) {
        ...
        // must have maintenance type selected
        if (!form.value.maintenance_type) return false
        // must have responsable selected
        if (!form.value.maintenance_responsible) return false
        // if external, must have company
        if (form.value.provider_type === 'external' && !form.value.provider_company) return false
        // must have warranty status
        if (form.value.has_warranty === null) return false
        // if warranty is true, must have warranty end date
        if (form.value.has_warranty === true && !form.value.warranty_end_date) return false
        // must have maintenance contract status
        if (form.value.maintenance_contract === undefined) return false
        // internal: responsable is enough (no company needed)
        return true
    }
    if (currentStep.value === 1) {
        // for normal flows, step1 (return location) is optional
        return true
    }
    return true
})
...
.summary-value.internal {
    color: #3b82f6;
}

.summary-value.external {
    color: #f59e0b;
}
...
.warranty-option .warranty-icon {
    font-size: 24px;
    transition: all 0.2s ease;
}

.warranty-option .warranty-icon.yes {
    color: #10b981;
}

.warranty-option .warranty-icon.no {
    color: #ef4444;
}

.warranty-option.selected .warranty-icon {
    font-size: 28px;
}
...
.contract-option .contract-icon.preventive {
    color: #3b82f6;
}

.contract-option .contract-icon.corrective {
    color: #f59e0b;
}

.contract-option .contract-icon.none {
    color: #6b7280;
}

.contract-option.selected .contract-icon {
    font-size: 28px;
}
...
/* Summary values for warranty and contract */
.summary-value.warranty-yes {
    color: #10b981;
}

.summary-value.warranty-no {
    color: #ef4444;
}

.summary-value.contract-preventive {
    color: #3b82f6;
}

.summary-value.contract-corrective {
    color: #f59e0b;
}
