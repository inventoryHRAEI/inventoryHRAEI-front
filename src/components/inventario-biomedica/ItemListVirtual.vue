  ...
  <div class="il-root">
    ...
    <div class="il-filters">
      ...
      <div class="il-table-filter-wrap">
        <button class="il-table-filter-btn" :class="{ active: showTableFilter || appliedValueFilters.length > 0 }" @click="toggleTableFilter">
          Filtrar
          <span v-if="appliedValueFilters.length > 0" class="il-table-filter-badge">{{ appliedValueFilters.length }}</span>
        </button>
        ...
        <div v-if="showTableFilter" class="il-table-filter-panel">
          ...
          <div class="il-table-filter-footer">
            <button class="il-table-filter-clear" @click="clearAppliedValueFilters">Limpiar</button>
    ...
    >
      ...
      >
        <div class="il-info">
          ...
          <div class="il-tags">
            <span class="il-tag-code">{{ getItemClave(item) }}</span>
            <span class="il-tag-stock">Stock: {{ getItemStock(item) }}</span>
          </div>
        </div>
        <div class="il-qty">
          <button
...
const appliedValueFilter = ref({ field: '', values: [] });

const appliedValueFilters = computed(() => {
  return Array.isArray(appliedValueFilter.value.values) ? appliedValueFilter.value.values : [];
});
...
watch(tableFilterField, (field) => {
  ...
  if (appliedValueFilter.value.field === field) {
    draftValueFilters.value = [...(appliedValueFilter.value.values || [])];
  } else {
...
const toggleTableFilter = () => {
  ...
  if (showTableFilter.value) {
    if (appliedValueFilter.value.field && tableFilterField.value !== appliedValueFilter.value.field) {
      tableFilterField.value = appliedValueFilter.value.field;
    }
    draftValueFilters.value = [...(appliedValueFilter.value.values || [])];
  }
};
...
const applyValueFilters = () => {
  appliedValueFilter.value = {
    field: tableFilterField.value,
    values: [...draftValueFilters.value]
  };
...
const clearAppliedValueFilters = () => {
  appliedValueFilter.value = { field: '', values: [] };
  draftValueFilters.value = [];
  visibleCount.value = CHUNK;
};
...
const filtered = computed(() => {
  ...
  if (appliedValueFilter.value.field && appliedValueFilters.value.length > 0) {
    const selected = new Set(appliedValueFilters.value);
    r = r.filter(item => {
      const valueKey = normalizeText(getFieldDisplayValue(item, appliedValueFilter.value.field));
