      ...
      <div class="greeting-bar">
        <div class="greeting-left">
          ...
          <div class="greeting-text">
            ...
            <h2 class="greeting-name">{{ isAdmin ? 'Panel Administrativo' : 'Panel de usuario' }}</h2>
...
const getCurrentRole = () => {
  if (typeof window !== 'undefined' && window.__USER_ROLE__) return String(window.__USER_ROLE__).toLowerCase().trim()
  try {
    const user = JSON.parse(localStorage.getItem('user') || 'null')
    if (user?.role) return String(user.role).toLowerCase().trim()
  } catch {}
  return String(localStorage.getItem('role') || 'user').toLowerCase().trim()
}
