        ...
        <div class="admin-users-container">
            ...
            <div class="hero-section">
                <div class="hero-content">
                    ...
                    <p class="hero-subtitle">Gestiona cuentas, permisos y acceso operativo del sistema</p>
                </div>
            ...
            <div class="stats-section">
                ...
                <div class="stat-card stat-card--access">
                    <span class="stat-label">Usuarios con acceso</span>
                    <span class="stat-number access">{{ accessUserCount }}</span>
                </div>
            </div>
            ...
            <div class="search-wrap">
                <div class="search-bar">
                    ...
                    <input v-model="searchQuery" type="text" placeholder="Buscar por nombre o correo..."
                        @input="filterUsers" />
                </div>
                <div class="role-filter-pills" role="tablist" aria-label="Filtro de roles">
                    <button class="filter-pill" :class="{ active: roleFilter === 'all' }" @click="setRoleFilter('all')">Todos</button>
                    <button class="filter-pill" :class="{ active: roleFilter === 'access' }" @click="setRoleFilter('access')">Usuarios con acceso</button>
                    <button class="filter-pill" :class="{ active: roleFilter === 'admin' }" @click="setRoleFilter('admin')">Administradores</button>
                </div>
                <div class="results-pill">Mostrando {{ filteredUsers.length }} de {{ users.length }}</div>
            </div>
            ...
            <div class="users-grid">
                ...
                <div v-for="user in filteredUsers" :key="user.id" class="user-card">
                    ...
                    <div class="card-header">
                        ...
                        <div class="role-badge" :class="roleBadgeClass(user.role)">
                            <svg v-if="roleBadgeClass(user.role) === 'admin'" width="16" height="16" viewBox="0 0 24 24"
                                fill="currentColor">
                                <path
                            ...
                            <span>{{ formatRoleUnified(user.role) }}</span>
                        </div>
                        <div class="status-badge" :class="statusBadgeClass(user)">
                            <span>{{ formatAccountStatus(user) }}</span>
                        </div>
                    </div>
                    ...
                    <div class="card-footer">
                        ...
                        <button v-if="!isRevokedUser(user) && (user.role === 'admin' || adminCount < 3)"
                            class="action-btn role-btn"
                            @click="changeRole(user)"
                            :disabled="user.role !== 'admin' && adminCount >= 3"
                            :title="user.role === 'admin' ? 'Degradar a usuario con acceso' : adminCount >= 3 ? 'Límite de 3 administradores alcanzado' : 'Promover a administrador'">
                            <svg v-if="user.role !== 'admin'" width="18" height="18" viewBox="0 0 24 24"
                                fill="currentColor">
                                <path
                                    d="M12 2L15.09 8.26H22L17.55 12.19L18.91 18.45L12 14.62L5.09 18.45L6.45 12.19L2 8.26H8.91L12 2Z" />
                            </svg>
                            ...
                            {{ user.role === 'admin' ? 'Hacer usuario' : 'Promover' }}
                        </button>
                        <button v-if="!isRevokedUser(user)" class="action-btn revoke-btn" @click="revokeUserAccess(user)" title="Revocar acceso">
                            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                                stroke-width="2">
                                <circle cx="12" cy="12" r="10" />
                        ...
                        <div class="detail-scroll">
                            ...
                            <div class="user-header-section">
                                ...
                                <div class="user-main-info">
                                    ...
                                    <span class="role-badge" :class="roleBadgeClass(selectedUser?.role)">
                                        <svg v-if="roleBadgeClass(selectedUser?.role) === 'admin'" width="14" height="14"
                                            viewBox="0 0 24 24" fill="currentColor">
                                            <path
                                                d="M12 2L15.09 8.26H22L17.55 12.19L18.91 18.45L12 14.62L5.09 18.45L6.45 12.19L2 8.26H8.91L12 2Z" />
                                        </svg>
                                        {{ formatRoleUnified(selectedUser?.role) }}
                                    </span>
                                    <span class="status-badge" :class="statusBadgeClass(selectedUser)">
                            ...
                            <div class="info-section">
                                ...
                                <div class="info-grid">
                                    ...
                                    <div class="info-item">
                                        <label>Rol</label>
                                        <span>{{ formatRoleUnified(selectedUser?.role) }}</span>
                                    </div>
                                    <div class="info-item">
                                        <label>Estado de cuenta</label>
                                        <span>{{ formatAccountStatus(selectedUser) }}</span>
                                    </div>
                                    <div class="info-item">
                            ...
                            <div class="info-section">
                                ...
                                <div class="stats-grid">
                                    ...
                                    <div class="stat-box">
                                        ...
                                        <span class="stat-value">{{ selectedUser?.role === 'admin' ? 'Administrador' : 'Usuario con acceso' }}</span>
                                    </div>
                                </div>
                            </div>
                        </div>
...
// Computadas
const adminCount = computed(() => users.value.filter(u => u.role === 'admin' && !isRevokedUser(u)).length)
const accessUserCount = computed(() => users.value.filter(u => (u.role === 'user' || u.role === 'privileged') && !isRevokedUser(u)).length)
...
function roleBadgeClass(role) {
    return role === 'admin' ? 'admin' : 'access'
}

function formatRoleUnified(role) {
    return role === 'admin' ? 'Administrador' : 'Usuario con acceso'
}

function normalizeAccountStatus(user) {
    if (!user) return 'unknown'
    const raw = String(user.account_status || '').trim().toLowerCase()
    if (raw) return raw
    return Number(user.verificado) === 1 ? 'approved' : 'pending'
}

function isRevokedUser(user) {
    return normalizeAccountStatus(user) === 'revoked'
}

function formatAccountStatus(user) {
    const status = normalizeAccountStatus(user)
...
function filterUsers() {
    ...
    if (roleFilter.value === 'admin') {
        list = list.filter((u) => u.role === 'admin' && !isRevokedUser(u))
    } else if (roleFilter.value === 'access') {
        list = list.filter((u) => (u.role === 'user' || u.role === 'privileged') && !isRevokedUser(u))
    }
...
const ROLES = {
    ...
    privileged: { label: 'Usuario con acceso', description: 'Cuenta validada por administración con acceso a la aplicación', color: '#38bdf8' },
    user: { label: 'Usuario con acceso', description: 'Cuenta activa con acceso a la aplicación', color: '#3b82f6' }
}

async function changeRole(user) {
    try {
        const currentRoleKey = user.role === 'privileged' ? 'user' : user.role
        ...
        const { value: newRole } = await Swal.fire({
            ...
            html: `
                <div style="text-align: left; margin-bottom: 20px;">
                    ...
                    <p style="margin-bottom: 20px;"><strong>Rol actual:</strong> <span class="role-badge-${currentRoleKey}">${ROLES[currentRoleKey]?.label || currentRoleKey}</span></p>
                </div>
                ...
                <select id="role-select" class="swal2-select" style="padding: 10px; font-size: 14px;">
                    <option value="user" ${currentRoleKey === 'user' ? 'selected' : ''}>Usuario con acceso - Uso operativo de la app</option>
                ...
                <div id="role-description" style="margin-top: 15px; padding: 12px; background: #f3f4f6; border-radius: 8px; font-size: 13px; color: #6b7280;">
                    ${ROLES[currentRoleKey]?.description || ROLES.user.description}
                </div>
            `,
            preConfirm: () => {
        ...
        if (!newRole || newRole === currentRoleKey) return
...
async function revokeUserAccess(user) {
    try {
        ...
        users.value = users.value.map((item) =>
            item.id === user.id ? { ...item, verificado: 0, account_status: 'revoked' } : item
        )
        if (selectedUser.value?.id === user.id) {
            selectedUser.value = { ...selectedUser.value, verificado: 0, account_status: 'revoked' }
        }
        filterUsers()
...
.admin-users-container {
    padding: 24px;
    background:
        radial-gradient(1200px 420px at 8% -18%, rgba(56, 189, 248, 0.18), transparent 58%),
        radial-gradient(900px 340px at 92% 0%, rgba(251, 191, 36, 0.12), transparent 62%),
        linear-gradient(135deg, #0b1220 0%, #111827 48%, #0e1a2f 100%);
    min-height: 100vh;
    border-radius: 16px;
}
...
.merge-note {
    ...
    background: linear-gradient(95deg, rgba(8, 145, 178, 0.18), rgba(30, 64, 175, 0.16));
...
.stat-card {
    background: linear-gradient(165deg, rgba(15, 23, 42, 0.86), rgba(30, 41, 59, 0.7));
...
.filter-pill.active {
    ...
    background: linear-gradient(135deg, rgba(14, 116, 144, 0.65), rgba(30, 64, 175, 0.6));
    color: #e0f2fe;
