...
const loadUserData = async () => {
  try {
    ...
    // Intentar cargar usuario desde endpoint /me (la forma correcta)
    let user = null
    let users = null
    ...
    try {
      ...
      if (resMe.ok) {
        ...
        console.warn('GET /me fallido, usando lista de usuarios')
      }
    } catch (err) {
      console.warn('Error /me, fallback a /users:', err)
    }
...
const handleSave = async () => {
  ...
  try {
    ...
    // Success: actualizar datos globales/localStorage para reflejar la nueva foto en toda la app
    const resp = await res.json()
    if (resp && resp.user) {
      // Construir usuario actualizado y preferir la preview para mostrarla inmediatamente
      const updatedUserFromResp = { ...(resp.user || {}) }
      // Si existe preview (data URL) la usamos para actualizar la UI inmediatamente
      if (previewUrl.value) {
        updatedUserFromResp.foto = previewUrl.value
      }
      // Persistir y exponer globalmente
      localStorage.setItem('user', JSON.stringify(updatedUserFromResp))
      window.__sessionUser = updatedUserFromResp
      // Despachar evento con payload para que App.vue lo consuma inmediatamente
      try {
        window.dispatchEvent(new CustomEvent('session:updated', { detail: updatedUserFromResp }))
      } catch (e) {
        // Fallback: evento sin payload
        try { window.dispatchEvent(new Event('session:updated')) } catch (err) { /* ignore */ }
      }
    } else {
      // Fallback: si no hay resp.user, intentar actualizar a partir del usuario en storage
      const existing = JSON.parse(localStorage.getItem('user') || '{}') || {}
      const fallback = { ...existing, nombre: formData.value.nombre }
      if (previewUrl.value) fallback.foto = previewUrl.value
      localStorage.setItem('user', JSON.stringify(fallback))
      window.__sessionUser = fallback
      try {
        window.dispatchEvent(new CustomEvent('session:updated', { detail: fallback }))
      } catch (e) { try { window.dispatchEvent(new Event('session:updated')) } catch (err) {} }
    }
