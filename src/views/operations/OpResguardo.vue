                ...
                <div class="op-card insumos op-resguardo-form" ref="rootRef">
                    <form @submit.prevent="onSubmit" class="form-grid" id="resguardo-form" novalidate>
                        ...
                        <div class="section-card combined-card">
                            ...
                            <div class="section-grid combined">
                                <div v-if="!isBaseReplaced('motivoResguardo')" :class="['field', diffFieldClass('motivoResguardo')]" style="grid-column: span 6;">
                                    <label>{{ getBaseLabel('motivoResguardo', 'Motivo de Resguardo') }}</label>
                                    <CustomSelect v-model="form.motivoResguardo" :options="motivoResguardoOptionsComputed"
                                        :placeholder="getBasePlaceholder('motivoResguardo', 'Seleccionar motivo')" />
                                </div>

                                <div v-if="form.motivoResguardo === 'otro' && !isBaseReplaced('otroMotivo')"
                                    :class="['field', diffFieldClass('otroMotivo')]" style="grid-column: span 6;">
                                    <label>{{ getBaseLabel('otroMotivo', 'Especifique Motivo de Resguardo') }}</label>
                                ...
                                <div v-if="!isBaseReplaced('descripcion')" :class="['field', diffFieldClass('descripcion')]" style="grid-column: 1 / -1;">
                                    <label>{{ getBaseLabel('descripcion', 'Descripción de Resguardo') }}</label>
...
const schemaSections = [
    { id: 'solicitante', title: 'Datos del Resguardante' },
    { id: 'motivo', title: 'Motivo y Descripción de Resguardo' },
    { id: 'equipos', title: 'Equipo Médico, Accesorio o Consumible en Resguardo' },
    { id: 'observaciones', title: 'Observaciones y Soporte' },
    { id: 'firmas', title: 'Firmas' }
]
const schemaOptionSets = [
    { key: 'motivoResguardo', label: 'Motivo de Resguardo' }
]
...
const motivoResguardoOptionsComputed = computed(() => {
    const override = formSchema.value?.optionSets?.motivoResguardo
    return Array.isArray(override) && override.length ? override : motivoResguardoOptions
})
...
function applySnapshotToForm(snapshot) {
    ...
    form.motivoResguardo = orden.motivo_resguardo || orden.motivo_entrada || ''
...
// motivoResguardoOptions importado arriba
...
watch(
    () => form.motivoResguardo,
    (val) => {
        if (val != null && val !== '' && form.motivoEntrada !== val) {
            form.motivoEntrada = val
        }
    }
)
...
watch(
    () => form.motivoEntrada,
    (val) => {
        if (val != null && val !== '' && form.motivoResguardo !== val) {
            form.motivoResguardo = val
        }
    }
)
...
async function generarExcelEntrada(payloadParam) {
    ...
    try {
        ...
        const motiveOptions = (motivoResguardoOptionsComputed.value || []).filter(o => o && o.value)
        const motivoValue = form.motivoResguardo || form.motivoEntrada
        const indiceMotivo = motiveOptions.findIndex(o => o.value === motivoValue)
...
async function onSubmit() {
    ...
    const motivoLabel = (() => {
        ...
        const opt = motivoResguardoOptionsComputed.value.find(o => o.value === form.motivoResguardo)
...
const loadOrderData = async () => {
    ...
    try {
        ...
        form.motivoResguardo = data.motivo_resguardo || data.motivoResguardo || data.motivo_entrada || data.motivoEntrada || ''
