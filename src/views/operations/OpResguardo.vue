<template>
  <FormShell>
    <template #title>Órdenes de Resguardo</template>
        ...
        <form @submit.prevent="onSubmit" class="form-grid" id="resguardo-form" novalidate>
          ...
          <div class="section-card combined-card">
            ...
            <div class="section-grid combined">
              ...
              <div class="field">
                <label>Descripción del Resguardo</label>
                <textarea
                  class="control"
          ...
          <div class="form-footer combined-card">
            <div class="form-actions">
              ...
              >
                ...
                {{ isSubmitting ? 'Guardando...' : 'Crear Orden de Resguardo' }}
              </button>
            </div>
          </div>
        </form>
...
const agregarItem = () => {
  ...
  form.equiposResguardo.push(itemToAdd)
...
const onTrashDone = (item) => {
  const idx = form.equiposResguardo.indexOf(item)
  if (idx > -1) form.equiposResguardo.splice(idx, 1)
}
...
const isFormValid = computed(() => {
  ...
  if (form.equiposResguardo.length === 0) return false
  return true
})
...
const onSubmit = async () => {
  ...
  try {
    const result = await Swal.fire({
      ...
      text: 'Se generará una nueva orden de resguardo con los datos ingresados.',
      icon: 'question',
    ...
    notifier.success('Orden de resguardo creada correctamente')
    router.push({ name: 'admin-dashboard' })
    
  } catch (error) {
    console.error(error)
    notifier.error('Error al crear la orden de resguardo')
  } finally {
...
/* Overrides específicos para resguardo */
.op-card.insumos {
  max-width: 100%;
}
