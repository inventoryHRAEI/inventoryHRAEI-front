...
import { INVENTARIO_FINAL_TABLE_COLUMNS } from '@/constants/inventarioFinalColumns.js'
import {
    ensureBiomedicalEquipmentCatalogLoaded,
    getBiomedicalEquipmentCatalogState,
    getBiomedicalEquipmentCatalogSnapshot,
    warmupBiomedicalEquipmentCatalog,
    invalidateBiomedicalEquipmentCatalog
} from '@/services/biomedicalEquipmentCatalog.js'
...
watch(activeTab, async (val) => {
  if (val === 'inventory') {
    try {
      const reusedWarmCatalog = await seedInventoryFromWarmCatalog()
      const haveLocal = Array.isArray(allData.value) && allData.value.length > 0
      if (!reusedWarmCatalog && !haveLocal) {
        console.log('[activeTab] inventory selected — loading equipment data')
        await runSearchBase(buildQueryParams, () => filters.value.estatus === SIN_ESTADO_VALUE, useServerPagination.value ? { serverSide: true, page: 1, pageSize: TABLE_PAGE_SIZE } : { limitReactive: isMobileOrNetworkNow() })
      }
    } catch (e) {
...
const { rows: biomedicalCatalogRows } = getBiomedicalEquipmentCatalogState()
...
// Invalidate any persisted catalog cache on mount to avoid mixing legacy data
onMounted(() => {
    try { invalidateBiomedicalEquipmentCatalog() } catch (e) {}
})
...
async function handleNeedMore() {
    try {
        ...
        if (filteredData.value.length >= (serverTotal.value || Infinity)) return
        // Trigger progressive full load on demand (single-flight handled by composable)
        await runSearchBase(buildQueryParams, () => filters.value.estatus === SIN_ESTADO_VALUE, { serverSide: true, full: true, pageSize: TABLE_PAGE_SIZE })
    } catch (e) {
        console.warn('[handleNeedMore] failed to request more items', e)
    }
}
...
async function seedInventoryFromWarmCatalog() {
    try {
        if (activeTab.value !== 'inventory' || hasFilters.value) return false

        let snapshot = getBiomedicalEquipmentCatalogSnapshot()
        if (!snapshot.rows.length) {
            await ensureBiomedicalEquipmentCatalogLoaded({ full: false })
            snapshot = getBiomedicalEquipmentCatalogSnapshot()
        }

        if (!snapshot.rows.length) return false

        const seededRows = snapshot.rows
        allData.value = seededRows
        filteredData.value = filters.value.estatus === SIN_ESTADO_VALUE
            ? seededRows.filter(it => !hasRealValue(it && it['ESTATUS']))
            : seededRows
        serverTotal.value = snapshot.total || seededRows.length
        metaError.value = ''
        loading.value = false

        if (!snapshot.fullyLoaded || !snapshot.isFresh) {
            warmupBiomedicalEquipmentCatalog()
        }

        return true
    } catch (e) {
        console.warn('[warm-catalog] No se pudo sembrar la tabla desde memoria:', e && e.message)
        return false
    }
}
...
function handleTableRefresh() {
    ...
    try {
        runSearchBase(buildQueryParams, () => filters.value.estatus === SIN_ESTADO_VALUE, { serverSide: true, page: 1, pageSize: TABLE_PAGE_SIZE })
    } catch (e) {
        console.warn('[BiomedicalTestingEnvironment] Error al refrescar tabla biomédica:', e && e.message)
    }
}

const tableColumns = computed(() => INVENTARIO_FINAL_TABLE_COLUMNS)

const tableDataSource = computed(() => {
    if (hasFilters.value) return filteredData.value

    const catalogRows = Array.isArray(biomedicalCatalogRows.value) ? biomedicalCatalogRows.value : []
    if (catalogRows.length > 0) return catalogRows

    return Array.isArray(allData.value) && allData.value.length > 0
        ? allData.value
        : filteredData.value
})
...
function getEquipmentName(item) {
    return getFirstExistingValue(item, ['EQUIPO MÉDICO', 'EQUIPO MEDICO', 'EQUIPO_MEDICO', 'EQUIPO', 'nombre'])
}

function getItemSearchTokens(item) {
    return [
        getInventoryNo(item),
        getSerialNo(item),
        getEquipmentName(item),
        getFirstExistingValue(item, ['MARCA']),
        getFirstExistingValue(item, ['MODELO'])
...
async function onScanCode(code) {
    try {
        ...
        const localItem = (allData.value || []).find(item => {
            const [itemCode, itemSerie, itemName, itemBrand] = getItemSearchTokens(item).map(v => v.toUpperCase())
        ...
        if (localItem) {
            console.log('[onScanCode] Found in local data:', getInventoryNo(localItem))
        ...
        try {
            ...
            if (item) {
                console.log('[onScanCode] lookup returned', getInventoryNo(item))
                handleShowHistoryPanel(item)
                return
            }
        } catch (lookupErr) {
        ...
        const flexibleItem = (allData.value || []).find(item => {
            const [itemCode, itemSerie, itemName, itemBrand] = getItemSearchTokens(item)
        ...
        if (flexibleItem) {
            console.log('[onScanCode] Found with flexible search:', getInventoryNo(flexibleItem))
        ...
        const emergencyItem = (allData.value || []).find(item => {
            const itemCode = String(getInventoryNo(item) || '').trim().toUpperCase()
...
function handleShowBarcode(item) {
    try {
        const code = getInventoryNo(item)
...
function handleShowUpdatePanel(item) {
    try {
        ...
        nextTick(() => {
            ...
            console.log('[handleShowUpdatePanel] opened modal for', getInventoryNo(item) || 'unknown')
        })
    } catch (e) {
...
function handleExternalScan(code) {
    if (!code) return
    const found = (allData.value || []).find(i => String(getInventoryNo(i) || '').trim() === String(code).trim()) || null
    if (found) {
...
function scheduleSearch() {
    ...
    searchTimer = setTimeout(() => {
        ...
        runSearchBase(buildQueryParams, () => filters.value.estatus === SIN_ESTADO_VALUE, useServerPagination.value ? { serverSide: true, page: 1, pageSize: TABLE_PAGE_SIZE, limitReactive: true, bypassCache: true } : { limitReactive: isMobileOrNetworkNow(), bypassCache: true })
        currentPage.value = 1
...
function queueInitialLoad() {
    const run = async () => {
        ...
        // Forzar server-side desde el primer load: evita el fetch masivo inicial
        // que puede tardar ~10-20s y además puede activar límites reactivos.
        ...
        if (activeTab.value === 'inventory') {
            const reusedWarmCatalog = await seedInventoryFromWarmCatalog()
            if (!reusedWarmCatalog) {
                await runSearchBase(buildQueryParams, () => filters.value.estatus === SIN_ESTADO_VALUE, preferServer ? { serverSide: true, page: 1, pageSize: TABLE_PAGE_SIZE, limitReactive: true } : { limitReactive: isMobileOrNetworkNow() })
            }
        } else {
...
onMounted(async () => {
    ...
    try {
      await import('@/utils/persistentCache.js')
      console.log('[init] Caché biomédica conservada para acelerar el primer render')
    } catch (e) {
      console.warn('[init] No se pudo preparar la caché persistente:', e && e.message)
    }
    ...
    initMaintenanceMap()
    try { warmupBiomedicalEquipmentCatalog() } catch (e) {}
    
    setTimeout(() => {
        contentReady.value = true
        // Default page size for traditional pagination
        try { pageSize.value = Number(pageSize.value) || TABLE_PAGE_SIZE } catch (e) {}
    }, 0) // Instant rendering
      ...
      if (typeof window !== 'undefined') {
        ...
        window.__BIOMED_TEST__.openBarcode = async (it) => {
          try {
            ...
            console.log('[__BIOMED_TEST__] openBarcode helper called for', getInventoryNo(item))
            // ensure values set and modal opened after DOM updates
            barcodeCode.value = String(getInventoryNo(item) || '')
            barcodeItem.value = item || null
            await nextTick()
            barcodeModalOpen.value = true
            return true
...
watch([currentPage, pageSize], () => {
    // La tabla ya pagina internamente; evitamos refetches redundantes por estado legado de cards.
})

// Si el servidor reporta un total muy grande, volver a la página 1 del estado legado.
...
watch(displayedCards, (cards) => {
    try {
        const codes = (cards || []).map(it => getInventoryNo(it)).filter(Boolean)
...
watch(filteredData, (newVal) => {
  try {
    ...
    const sample = Array.isArray(newVal) ? newVal.slice(0,6).map(it => ({ 'No. DE INVENTARIO': getInventoryNo(it), 'EQUIPO MÉDICO': getEquipmentName(it) })) : newVal
