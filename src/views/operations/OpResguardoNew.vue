                                        ...
                                        <div v-for="(unidad, idx) in newItem.unidades" :key="idx" class="unit-card">
                                            ...
                                            <div class="unit-fields">
                                                <template v-if="isExternalGoods">
                                                    ...
                                                    <div v-if="newItem.tipo === 'refaccion'" class="searchable-field">
                                                        <label class="mini-label">Descripción</label>
                                                        <input v-model.trim="unidad.descripcion" type="text" class="form-input"
                                                            placeholder="Describe la refacción..." autocomplete="off"
                                                            autocorrect="off" autocapitalize="off" spellcheck="false" />
                                                    </div>
...
function isCompleteManualUnit(unit) {
    const internalItem = belongsToHospital.value === true

    if (isManualFurnitureType(newItem.tipo)) {
        const claveOk = !internalItem || hasTrimmedValue(unit.claveHRAEI)
        return hasTrimmedValue(unit.nombre) &&
            hasTrimmedValue(unit.marca) &&
            hasTrimmedValue(unit.modelo) &&
            hasTrimmedValue(unit.ubicacion) &&
            hasTrimmedValue(unit.referencia) &&
            claveOk
    }

    if (isManualRefaccionType(newItem.tipo)) {
        const claveOk = !internalItem || hasTrimmedValue(unit.claveHRAEI)
        return hasTrimmedValue(unit.nombre) &&
            hasTrimmedValue(unit.descripcion) &&
            hasTrimmedValue(unit.marca) &&
            hasTrimmedValue(unit.modelo) &&
            hasTrimmedValue(unit.ubicacion) &&
            hasTrimmedValue(unit.equipoAsociado) &&
            hasTrimmedValue(unit.referencia) &&
            claveOk
    }
