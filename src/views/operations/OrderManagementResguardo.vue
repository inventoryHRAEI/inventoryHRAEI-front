    ...
    <div class="order-management-container">
        <ActionPanel class="order-management-main">
                ...
                <div class="title-row">
                    ...
                    <details v-if="isAdmin" ref="archiveMenuRef" class="archive-menu">
                        <summary class="btn-reset-folios archive-summary" :class="{ 'viewing-archived': viewingArchived }" title="Opciones de archivo">
                            <svg v-if="!viewingArchived && !archivedStore.hasArchivedByType.resguardo" xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                                <polyline points="21 8 21 21 3 21 3 8"></polyline>
                                <rect x="1" y="3" width="22" height="5"></rect>
                                <line x1="10" y1="12" x2="14" y2="12"></line>
                            </svg>
                            <svg v-else-if="!viewingArchived && archivedStore.hasArchivedByType.resguardo" xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                        ...
                        <div class="archive-popover">
                            <button type="button" class="archive-action-item" :disabled="isResetting" @click="handleArchiveAction('resguardo')">
                                Archivar y reiniciar folios
                            </button>
                            <button type="button" class="archive-action-item secondary" @click="openArchivedPanel('resguardo')">
                                Ver archivadas ({{ archivedStore.archivedCounts.resguardo || 0 }})
                            </button>
                        </div>
                    </details>
                        ...
                        <div style="display: flex; gap: 8px; align-items: center;">
                            <div style="flex: 1;">
                                ...
                                <template v-else-if="f.key === 'motivo'">
                                    <CustomSelect v-model="filterMotivo" :options="motivoResguardoOptions"
            ...
            <OrdersTable :filtered-orders="filteredOrders" :show-column-service="showColumnService"
                ...
                :can-edit-specific-order="canEditSpecificOrder"
                :can-delete-order="canDeleteOrder"
                :on-request-edit="handleRequestEdit"
                ...
                <div v-if="showWizardModal" class="wizard-modal-overlay" @click.self="requestCloseWizard">
                    <div class="wizard-modal-container">
                        ...
                        <div class="wizard-modal-content">
                            <OpResguardo v-if="showWizardModal" :key="`wizard-${wizardRefreshKey}`" ref="wizardResguardoRef"
                                @created="onWizardCreated" @cancel="closeWizardImmediately" />
                        </div>
                    </div>
                </div>
            </Transition>
        ...
        <ModalBase :open="showEditModal" @close="handleModalClose" @close-request="handleModalClose" :maxWidth="1100"
            ...
            <div class="om-edit-shell">
                ...
                <div v-if="activeTab !== 'main'" class="om-version-panel" role="tabpanel">
                    ...
                    <OpResguardo v-if="isActiveTabEditable" :key="`edit-${selectedOrderId}-${activeTab}`" :modo="'editar'"
                    ...
                    <OpResguardo v-else :key="`snap-${selectedOrderId}-${activeTab}`" :modo="'editar'"
                ...
                <div v-show="activeTab === 'main' && !branchTabs.length" role="tabpanel" class="om-main-panel">
                    <OpResguardo :key="mainEditorKey" ref="OpResguardoRef" :ordenId="selectedOrderId" :modo="'editar'"
                                ...
                                <li v-for="v in docVersions" :key="v.id" class="doc-list-item"
                                    ...
                                    <button class="doc-item-btn" @click="selectDocVersion(v)">
                                        ...
                                        <div class="doc-item-content">
                                            <div class="doc-item-name">{{ v.name || ('resguardo ' + docTitle + (v.version
                                                ? `
                                                v${v.version}` : '') + '.pdf') }}</div>
...
import { confirmDelete, confirmCancel, confirmReset, showSuccess, showError, showAlert, showLoading, closeModal } from '@/utils/sweetAlertConfig'
...
const approvedEditOrderIds = ref(new Set())

function normalizeOrderId(orderOrId) {
    const raw = (typeof orderOrId === 'object' && orderOrId !== null)
        ? (orderOrId.folio ?? orderOrId.id ?? '')
        : (orderOrId ?? '')
    return String(raw).trim()
}

function canEditSpecificOrder(order) {
    if (canEditOrder.value) return true
    const orderId = normalizeOrderId(order)
    if (!orderId) return false
    return approvedEditOrderIds.value.has(orderId)
}

async function refreshApprovedEditOrders() {
    ...
    try {
        ...
        const approved = new Set(
            (Array.isArray(requests) ? requests : [])
                .filter(req => String(req?.operation_type || '').toLowerCase() === 'resguardo' && String(req?.status || '').toLowerCase() === 'approved')
                .map(req => normalizeOrderId(req?.operation_id))
                .filter(Boolean)
        )
        approvedEditOrderIds.value = approved
    } catch (error) {
        console.warn('[OrderManagementResguardo] No se pudieron refrescar solicitudes de edición aprobadas', error)
    }
}
...
const handleRequestEdit = async (order) => {
  ...
  try {
    ...
    if (res.ok) {
      notifier.success('Solicitud de edición enviada correctamente')
    } else {
      const err = await res.json()
      notifier.error(err.error || 'Error al enviar solicitud')
    }
  } catch (err) {
    notifier.error('Error de red al enviar solicitud')
    console.error(err)
  }
}
...
onMounted(() => {
    ...
    try { 
        ...
        const path = route && route.fullPath ? String(route.fullPath) : ''
        console.debug('[OrderManagementResguardo] mounted, dispatching route:mounted', { name, path }); 
        window.dispatchEvent(new CustomEvent('route:mounted', { detail: { name, path } })) 
    } catch (e) {}
    // Cargar órdenes archivadas para reactividad
    try { archivedStore.fetchArchived('resguardo').catch(() => {}) } catch (e) {}
    try { refreshApprovedEditOrders().catch(() => {}) } catch (e) {}
    ...
    try {
        ...
        if (canRestoreWizard) {
            ...
            clearWizardState()
        } else if (hasResguardoDraft && restoreState && String(restoreState.route || '').includes('resguardo')) {
            wizardType.value = 'resguardo'
            showWizardModal.value = true
        }
...
const wizardType = ref('resguardo')
const wizardRefreshKey = ref(0)
const wizardResguardoRef = ref(null)
const isWizardRefreshing = ref(false)

watch([showWizardModal, wizardType], ([open, type]) => {
    try {
        if (open) {
            saveWizardState({ routeName: route.name, type: type || 'resguardo' })
        } else {
            clearWizardState()
        }
    } catch (e) {}
...
// Wrapper que llama al helper compartido y pasa `archivedPanelRef`
async function confirmAndResetFolios(orderType = 'resguardo') {
    if (isResetting.value) return
    isResetting.value = true
    try {
        const res = await resetFoliosHelper(orderType, { archivedPanelRef })
        try { await reloadOrdersFromServer() } catch (e) { console.warn('[OrderManagementResguardo] reload after reset failed', e) }
        return res
    } catch (e) {
        console.warn('[OrderManagementResguardo] confirmAndResetFolios failed:', e)
    } finally {
...
const archiveButtonLabel = computed(() => {
    const count = Number(archivedStore.archivedCounts.resguardo || 0)
...
function openArchivedPanel(orderType = 'resguardo') {
    closeArchiveMenu()
...
async function handleArchiveAction(orderType = 'resguardo') {
    closeArchiveMenu()
...
async function openDocumentModal(order) {
    ...
    const type = (order?.orderType || order?.type || order?.tipo || 'resguardo').toString().toLowerCase()
...
async function fetchDocVersionsFor(folio, orderType = 'resguardo') {
    ...
    try {
        const endpointPrefix = orderType || 'resguardo'
        ...
        try {
            ...
            if (versRes.ok) {
                ...
                const versItems = rows.map(v => {
                    ...
                    return {
                        id: `ver-${ver}`,
                        name: `resguardo ${folio} v${ver}.pdf`,
                        version: ver,
                        type: 'version',
                        createdAt: createdAt ? new Date(createdAt).toISOString() : null,
    ...
    if (result.isConfirmed) {
        ...
        if (folio) {
            try {
                ...
                const delRes = await fetch(`/api/ops/resguardo/${encodeURIComponent(folio)}`, { method: 'DELETE', headers })
                if (!delRes.ok) {
                    let body = null
        ...
        try { await reloadOrdersFromServer(); await showSuccess('Cancelada', 'La orden ha sido cancelada correctamente.') } catch (e) {
            const localOrder = allOrders.value.find(o => o.id === orderId)
            if (localOrder) localOrder.status = 'cancelled'
            try { const raw = localStorage.getItem('orders_list'); if (raw) { const arr = JSON.parse(raw); const idx = arr.findIndex(o => String(o.id) === String(orderId)); if (idx !== -1) { arr[idx].status = 'cancelled'; localStorage.setItem('orders_list', JSON.stringify(arr)) } } } catch (e) {}
            await showSuccess('Cancelada', 'La orden ha sido cancelada (marcada localmente).')
        }
    }
    }

async function fetchSignedDocuments(folio, orderType = 'resguardo') {
    const endpointPrefix = orderType || 'resguardo'
...
async function submitUpload() {
    ...
    try {
        ...
        const res = await fetch(`/api/ops/resguardo/${encodeURIComponent(uploadOrderFolio.value)}/pdfs/upload`, fetchOptions)
...
const activeFiltersList = computed(() => {
    ...
    if (filterMotivoActive.value) list.push({ key: 'motivo', label: 'Motivo de Resguardo', type: 'select', bindings: { modelValue: filterMotivo, 'onUpdate:modelValue': val => filterMotivo.value = val, class: 'control filter-input' } })
...
const hasActiveFilters = computed(() => {
    return (
        ...
        filterClaveHRAEI.value || filterNoInventario.value || filterNoInventarioActive.value
    )
})
...
const filteredOrders = computed(() => {
    return allOrders.value.filter(order => {
        ...
        const matchMotivo = !filterMotivoActive.value || !filterMotivo.value || ((order.motivoResguardo || '').toLowerCase() === filterMotivo.value.toLowerCase())
        ...
        const matchTipo = !filterTipoActive.value || !filterTipo.value || (order.equiposResguardo || []).some(e => e.tipo === filterTipo.value)
        const matchItemText = !filterItemTextActive.value || !filterItemText.value || (order.equiposResguardo || []).some(e => {
        ...
        const matchMarca = !filterMarcaActive.value || !filterMarca.value || (order.equiposResguardo || []).some(e => {
            const search = filterMarca.value.toLowerCase()
            return String(e.marca || '').toLowerCase().includes(search)
        })
        const matchModelo = !filterModeloActive.value || !filterModelo.value || (order.equiposResguardo || []).some(e => {
            const search = filterModelo.value.toLowerCase()
            return String(e.modelo || '').toLowerCase().includes(search)
        })
        const matchUbicacion = !filterUbicacionActive.value || !filterUbicacion.value || (order.equiposResguardo || []).some(e => {
            const search = filterUbicacion.value.toLowerCase()
            return String(e.ubicacion || '').toLowerCase().includes(search)
        })
        const matchNoInventario = !filterNoInventarioActive.value || !filterNoInventario.value || (order.equiposResguardo || []).some(e => {
        ...
        const matchClaveHRAEI = !filterClaveHRAEIActive.value || !filterClaveHRAEI.value || (order.equiposResguardo || []).some(e => {
            const search = filterClaveHRAEI.value.toLowerCase()
            return String(e.claveHRAEI || '').toLowerCase().includes(search)
        })
        const matchAccesorio = !filterAccesorioActive.value || !filterAccesorio.value || (order.equiposResguardo || []).some(e => {
        ...
        const itemCount = (order.equiposResguardo || []).length || 0
...
function discardCurrentWizardDraft() {
    ...
    try { deleteDraftRemote(wizardType.value) } catch (e) { console.warn('[OrderManagementResguardo] failed to delete remote draft for', wizardType.value, e) }
}
...
async function handleWizardRefresh() {
    ...
    try {
        const target = wizardResguardoRef.value
...
async function openEditModal(order) {
    try { await refreshApprovedEditOrders() } catch (e) {}

    if (!canEditSpecificOrder(order)) {
        Swal.fire({
            title: 'Acceso denegado',
            text: 'No tienes permisos para editar esta orden. Si ya solicitaste edición, espera la aprobación del administrador.',
            icon: 'warning',
...
async function loadOrderHistoryAndVersions(folio) {
    ...
    try {
        ...
        if (versRes.ok) {
            ...
            // snapshots por versión (para render tipo OpResguardo)
            const snapMap = {}
...
function buildHighlights(rows) {
    ...
    const fieldMap = {
        ...
        motivo_resguardo: 'motivoResguardo',
...
async function persistEditedOrder(payload) {
    try {
        ...
        try {
            const curRes = await fetch(`/api/ops/resguardo/${encodeURIComponent(String(folio))}`, { cache: 'no-store' })
            if (curRes && curRes.ok) {
                ...
                fillIfEmpty('motivoResguardo', 'motivo_resguardo')
                ...
                // For equiposResguardo, prefer payload if provided, otherwise reconstruct from server items
                if (!Array.isArray(mergedPayload.equiposResguardo) || !mergedPayload.equiposResguardo.length) {
                    try {
                        const items = Array.isArray(curBody.items) ? curBody.items : []
                        // map DB items to frontend equiposResguardo shape
        ...
        console.log('[OrderManagement] Sending PUT /resguardo payload.signatures =', mergedPayload.signatures)
        const res = await fetch(`/api/ops/resguardo/${encodeURIComponent(String(folio))}`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
        ...
        try {
            const genRes = await fetch(`/api/ops/resguardo/${encodeURIComponent(folio)}/pdfs/generate-force`)
            if (genRes && genRes.ok) {
...
async function reloadOrdersFromServer() {
    loading.value = true
    try {
        const res = await fetch('/api/ops/resguardo/list', { cache: 'no-store' })
        if (!res.ok) {
            allOrders.value = []
            return
        }
        ...
        allOrders.value = items.map((wrapper) => {
            ...
            const equiposResguardo = orderItems.map(item => ({
            ...
            return {
                ...
                motivoResguardo: orden.motivo_resguardo || 'N/A',
                motivoEntrada: orden.motivo_resguardo || orden.motivoResguardo || 'N/A',
                ...
                equiposResguardo,
                equiposEntrada: equiposResguardo,
...
async function loadDocumentCountsForOrders() {
    ...
    const documentCountPromises = allOrders.value.map(async (order) => {
        try {
            const res = await fetch(`/api/ops/resguardo/${encodeURIComponent(order.folio)}/signed-documents`)
            if (res.ok) {
                const json = await res.json()
                const count = (json && json.documents && Array.isArray(json.documents)) ? json.documents.length : 0
...
async function handleDeleteMultipleWithModal(orderIds) {
    ...
    if (result.isConfirmed) {
        ...
        const successMsg = `${count} orden${count !== 1 ? 'es' : ''} ha${count !== 1 ? 'n' : ''} sido cancelada${count !== 1 ? 's' : ''} correctamente.`
        showSuccess('Canceladas', successMsg)
    }
}
...
async function deleteMultipleOrders(orderIds) {
    ...
    const results = await Promise.allSettled(Object.keys(folioMap).map(async (folio) => {
        try {
            const res = await fetch(`/api/ops/resguardo/${encodeURIComponent(folio)}`, { method: 'DELETE', headers })
...
async function tryPersistUpdatedOrder(order) {
    try {
        const res = await fetch('/api/ops/resguardo', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
...
const orderSearchFilters = {
    ...
    suggestionsByField: computed(() => {
        ...
        const getItems = (order) => Array.isArray(order.equiposEntrada) ? order.equiposEntrada : Array.isArray(order.equiposSalida) ? order.equiposSalida : []
        ...
        const flatItems = (selector) => orders.flatMap(order => selector(getItems(order), order) || [])
        ...
        addBucket('motivo', orders.map(order => order.motivoResguardo || order.motivoEntrada || order.motivoSalida || order.motivoServicio || order.motivo))
        addBucket('obs', orders.map(order => order.observaciones))
        addBucket('ing', orders.map(order => order.nombreIngeniero))
        addBucket('itemText', flatItems(items => items.flatMap(item => [
        ...
        addBucket('accesorio', flatItems(items => items.filter(item => String(item.tipo || '').toLowerCase().includes('acces')).map(item => item.descripcion || item.nombre || item.label)))
        addBucket('tipo', flatItems(items => items.map(item => item.tipo)))
        addBucket('marca', flatItems(items => items.map(item => item.marca)))
        addBucket('modelo', flatItems(items => items.map(item => item.modelo)))
        addBucket('ubicacion', flatItems(items => items.map(item => item.ubicacion)))
        addBucket('claveHRAEI', flatItems(items => items.map(item => item.claveHRAEI || item.clave_hraei || item.clave)))
        addBucket('noInventario', flatItems(items => items.map(item => item.noInventario || item.no_inventario || item.inventario)))
        return buckets
    })
}
...
function addEditItem() {
    ...
    if (!editingOrder.value.equiposResguardo) editingOrder.value.equiposResguardo = []
    editingOrder.value.equiposResguardo.push(copy)
    // reset
...
async function removeEditItem(idx) {
    ...
    if (result.isConfirmed) {
        editingOrder.value.equiposResguardo.splice(idx, 1)
    }
}
...
/* Estilos compatibles con OpResguardo en la modal */
.op-resguardo-modal-full .section-card {
    background: rgba(255, 255, 255, 0.04);
    border: 1px solid rgba(255, 255, 255, 0.08);
    border-radius: 12px;
    padding: 24px;
...
.op-resguardo-modal-full .section-card:hover {
    background: rgba(255, 255, 255, 0.05);
    border-color: rgba(46, 221, 90, 0.15);
}

.op-resguardo-modal-full .section-card.combined-card {
    display: flex;
    flex-direction: column;
}

.op-resguardo-modal-full .section-head {
    margin-bottom: 16px;
    display: flex;
    flex-direction: column;
    gap: 6px;
...
.op-resguardo-modal-full .section-head h4 {
    margin: 0;
    font-size: 1.1rem;
    font-weight: 700;
...
.op-resguardo-modal-full .section-head .hint {
    margin: 0;
    font-size: 0.8rem;
    color: rgba(255, 255, 255, 0.6);
    font-weight: 400;
...
.op-resguardo-modal-full .section-grid {
    display: grid;
    grid-template-columns: repeat(6, 1fr);
    gap: 16px;
}

.op-resguardo-modal-full .section-grid.combined {
    grid-template-columns: repeat(6, 1fr);
}

.op-resguardo-modal-full .field {
    display: flex;
    flex-direction: column;
    gap: 8px;
}

.op-resguardo-modal-full .field label {
    font-size: 0.85rem;
    font-weight: 600;
    color: rgba(255, 255, 255, 0.85);
    text-transform: uppercase;
...
.op-resguardo-modal-full .control,
.op-resguardo-modal-full input[type="text"],
.op-resguardo-modal-full input[type="date"],
.op-resguardo-modal-full input[type="time"],
.op-resguardo-modal-full textarea,
.op-resguardo-modal-full select {
    padding: 12px 18px;
    background: rgba(255, 255, 255, 0.05);
    border: 1px solid rgba(255, 255, 255, 0.12);
    border-radius: 10px;
...
.op-resguardo-modal-full .control::placeholder,
.op-resguardo-modal-full input::placeholder,
.op-resguardo-modal-full textarea::placeholder {
    color: rgba(255, 255, 255, 0.4);
}

.op-resguardo-modal-full .control:focus,
.op-resguardo-modal-full input:focus,
.op-resguardo-modal-full textarea:focus,
.op-resguardo-modal-full select:focus {
    outline: none;
    background: rgba(255, 255, 255, 0.08);
...
.op-resguardo-modal-full .control:disabled,
.op-resguardo-modal-full input:disabled,
.op-resguardo-modal-full textarea:disabled,
.op-resguardo-modal-full select:disabled {
