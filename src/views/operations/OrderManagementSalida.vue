    ...
    try {
        ...
        const approved = new Set(
            (Array.isArray(requests) ? requests : [])
                .filter(req => String(req?.operation_type || '').toLowerCase() === 'salida' && String(req?.status || '').toLowerCase() === 'approved')
                .map(req => normalizeOrderId(req?.operation_id))
                .filter(Boolean)
        )
        approvedEditOrderIds.value = approved
    } catch (error) {
        console.warn('[OrderManagementSalida] No se pudieron refrescar solicitudes de edición aprobadas', error)
    }
}
...
const handleRequestEdit = async (order) => {
  try {
    const res = await authedFetch(`/api/edit-requests`, {
      ...
      body: JSON.stringify({ operation_type: 'salida', operation_id: order.folio })
    })
