...
const motivoEntradaOptions = [
  ...
  { value: 'mantenimiento-correctivo', label: 'CORRECTIVO' },
...
const onSubmit = async () => {
  ...
  try {
    const result = await Swal.fire({
      ...
      text: 'Se generará una nueva orden de servicio con los datos ingresados.',
      icon: 'question',
    ...
    notifier.success('Orden de servicio creada correctamente')
    router.push({ name: 'admin-dashboard' })
    
  } catch (error) {
    console.error(error)
    notifier.error('Error al crear la orden de servicio')
  } finally {
...
/* Overrides específicos para servicio */
.op-card.insumos {
  max-width: 100%;
}
