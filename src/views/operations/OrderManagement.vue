    ...
    <div class="order-management-container">
        <ActionPanel class="order-management-main">
                ...
                <div class="title-row">
                    ...
                    <details v-if="isAdmin" ref="archiveMenuRef" class="archive-menu">
                        <summary class="btn-reset-folios archive-summary" :class="{ 'viewing-archived': viewingArchived }" title="Opciones de archivo">
                            ...
                            <svg v-else-if="!viewingArchived && archivedStore.hasArchivedByType.entrada" xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                        ...
                        <div class="archive-popover">
                            <button type="button" class="archive-action-item" :disabled="isResetting" @click="handleArchiveAction('entrada')">
                                Archivar y reiniciar folios
                            </button>
                            <button type="button" class="archive-action-item secondary" @click="openArchivedPanel('entrada')">
                                Ver archivadas ({{ archivedStore.archivedCounts.entrada || 0 }})
                            </button>
                        </div>
                    </details>
            ...
            <OrdersTable :filtered-orders="filteredOrders" :show-column-service="showColumnService"
                ...
                @generateSalida="handleGenerateSalida" @generateEntrada="handleGenerateEntrada" />
        ...
        <!-- Modal: Wizard de creación de orden (entrada o salida) -->
        <Teleport to="body">
    ...
    try {
        ...
        const approved = new Set(
            (Array.isArray(requests) ? requests : [])
                .filter(req => String(req?.operation_type || '').toLowerCase() === 'entrada' && String(req?.status || '').toLowerCase() === 'approved')
                .map(req => normalizeOrderId(req?.operation_id))
                .filter(Boolean)
        )
        approvedEditOrderIds.value = approved
    } catch (error) {
        console.warn('[OrderManagement] No se pudieron refrescar solicitudes de edición aprobadas', error)
    }
}
...
const archiveButtonLabel = computed(() => {
    const count = Number(archivedStore.archivedCounts.entrada || 0)
...
// Wrapper para reiniciar folios desde la plantilla
const isResetting = ref(false)
async function confirmAndResetFolios(orderType = 'entrada') {
    if (isResetting.value) return
    isResetting.value = true
    try {
        const res = await resetFoliosHelper(orderType, { archivedPanelRef })
        try { await reloadOrdersFromServer() } catch (e) { console.warn('[OrderManagement] reload after reset failed', e) }
        return res
    } catch (e) {
        console.warn('[OrderManagement] reset folios failed', e)
    } finally {
...
async function handleArchiveAction(orderType = 'entrada') {
    closeArchiveMenu()
...
onMounted(() => {
    ...
    try { archivedStore.fetchArchived('entrada').catch(() => {}) } catch (e) {}
...
watch([showWizardModal, wizardType], ([open, type]) => {
    try {
        if (open) {
            saveWizardState({ routeName: route.name, type: type || 'entrada' })
        } else {
            clearWizardState()
        }
    } catch (e) {}
...
async function fetchDocVersionsFor(folio, orderType = 'entrada') {
    ...
    try {
        ...
        if (res && res.ok) {
            try {
                ...
                const pdfItems = rows.map(p => {
                    const id = p.id || p.file_id || p.name || `${folio}-${Math.random().toString(36).slice(2,8)}`
                    const createdAt = p.created_at || p.createdAt || p.uploadedAt || null
                    const previewUrl = p.previewUrl || p.preview_url || `/api/ops/${endpointPrefix}/${encodeURIComponent(folio)}/pdfs/${encodeURIComponent(id)}/preview`
                    const downloadUrl = p.downloadUrl || p.download_url || `/api/ops/${endpointPrefix}/${encodeURIComponent(folio)}/pdfs/${encodeURIComponent(id)}/download`
                    return {
                        id,
                        name: p.name || (`entrada ${folio}` + (p.version ? ` v${p.version}` : '') + '.pdf'),
                ...
                docVersions.value = pdfItems
            } catch (e) {
                console.warn('Error parsing pdfs list response', e)
            }
        }

        // Also fetch snapshot versions and merge
        try {
            const versRes = await fetch(`/api/ops/${endpointPrefix}/${encodeURIComponent(folio)}/versions?limit=50`)
            if (versRes.ok) {
...
const filteredOrders = computed(() => {
    return allOrders.value.filter(order => {
        ...
        const matchNoInventario = !filterNoInventarioActive.value || !filterNoInventario.value || (order.equiposEntrada || []).some(e => {
...
function discardCurrentWizardDraft() {
    ...
    try { deleteDraftRemote(wizardType.value) } catch (e) { console.warn('[OrderManagement] failed to delete remote draft for', wizardType.value, e) }
}
    ...
    try {
        ...
        allOrders.value = items.map((wrapper) => {
            ...
            const equiposEntrada = orderItems.map(item => {
                const isExternalFlag = Boolean(
                    item?.isExternal ?? item?.is_external ?? item?.es_externo ??
                    (item?.pertenece_al_hospital === false) ?? (item?.perteneceAlHospital === false) ?? (item?.belongsToHospital === false)
                )
                return {
                    ...
                    // Propagar flags que el backend pueda devolver para detectar equipos externos/manuales
                    isExternal: isExternalFlag,
                    is_external: item?.is_external ?? item?.es_externo ?? null,
                    pertenece_al_hospital: item?.pertenece_al_hospital ?? null,
                    perteneceAlHospital: item?.perteneceAlHospital ?? null,
...
async function deleteMultipleOrders(orderIds) {
    const count = orderIds.length
    // Collect orders and folios to delete
    const targets = allOrders.value.filter(o => orderIds.includes(o.id))
    const folioMap = {}
    targets.forEach(t => {
        const folio = t.folio || (t.orden && t.orden.folio)
        if (!folio) return
        const typeStr = (t.orderType || t.type || t.tipo || 'entrada').toString().toLowerCase()
        const baseType = typeStr.includes('salida') ? 'salida' : typeStr.includes('resguardo') ? 'resguardo' : typeStr.includes('servicio') ? 'servicio' : 'entrada'
        folioMap[folio] = { id: t.id, baseType }
    })

    const token = localStorage.getItem('token')
    const headers = token ? { 'Authorization': `Bearer ${token}` } : {}

    // Attempt server deletes in parallel and keep track of successes
    const results = await Promise.allSettled(Object.keys(folioMap).map(async (folio) => {
        try {
            const { baseType } = folioMap[folio]
            const res = await fetch(`/api/ops/${baseType}/${encodeURIComponent(folio)}`, { method: 'DELETE', headers })
    ...
    const succeededIds = succeededFolios.map(f => (folioMap[f] && folioMap[f].id)).filter(Boolean)

    // If any succeeded, refresh from server to reflect current statuses; otherwise try to mark locally
    try {
    ...
    // Fallback: mark locally as cancelled for succeeded folios
    allOrders.value.forEach(o => {
        if (succeededIds.includes(o.id)) o.status = 'cancelled'
    })
    try {
    ...
    try {
        ...
        if (raw) {
            const arr = JSON.parse(raw)
            const updated = arr.filter(o => !succeededIds.includes(String(o.id)))
    ...
    if (failed.length) {
        await showAlert({ title: 'Atención', text: `${failed.length} eliminación(es) fallaron. Revisa permisos/conectividad.`, icon: 'warning' })
    }
}
...
const orderSearchFilters = {
    ...
    suggestionsByField: computed(() => {
        ...
        addBucket('motivo', orders.map(order => order.motivoEntrada || order.motivoSalida || order.motivoServicio || order.motivo))
...
// Heurística para detectar items externos si no cuentan con flag explícito
...
/**
 * Maneja la generación de una orden de SALIDA (baja) pre-llenada
 * desde una orden que contiene equipos externos
 */
function handleGenerateSalida(order) {
    if (!order) return
    
    // Extraer solo los items externos de la orden (usar heurística cuando no exista flag)
    const externalItems = (order.equiposEntrada || []).filter(item =>
        isItemExternalHeuristic(item)
    )
    
    if (externalItems.length === 0) {
        Swal.fire({
            icon: 'warning',
            title: 'Sin equipos externos',
            text: 'Esta orden no contiene equipos externos para generar una salida.',
            ...darkThemeConfig
        })
        return
    }
    
    // Preparar items pre-llenados para el wizard de salida
    wizardInitialItems.value = externalItems.map(item => ({
        ...
        isExternal: true,
        sourceOrder: order.folio
    }))
    
    wizardType.value = 'salida'
    showWizardModal.value = true
    
    console.debug('[OrderManagement] Generando salida pre-llenada desde orden:', order.folio, 'con', externalItems.length, 'items externos')
}

/**
 * Maneja la generación de una orden de ENTRADA pre-llenada
 * desde una orden que contiene equipos externos
 */
function handleGenerateEntrada(order) {
    if (!order) return
    ...
    if (externalItems.length === 0) {
        Swal.fire({
            icon: 'warning',
            title: 'Sin equipos externos',
            text: 'Esta orden no contiene equipos externos para generar una entrada.',
            ...darkThemeConfig
        })
        return
    }
    
    // Preparar items pre-llenados para el wizard de entrada
    wizardInitialItems.value = externalItems.map(item => ({
        tipo: item.tipo || 'equipo-medico',
        cantidad: item.cantidad || 1,
        descripcion: item.descripcion || item.nombre || '',
    ...
    wizardType.value = 'entrada'
    showWizardModal.value = true
    
    console.debug('[OrderManagement] Generando entrada pre-llenada desde orden:', order.folio, 'con', externalItems.length, 'items externos')
}
...
.btn-create-order {
    ...
    gap: 8px;
    padding: 8px 14px;
    background: linear-gradient(135deg, rgba(46, 221, 90, 0.9), rgba(34, 197, 94, 0.85));
    color: #fff;
    ...
    box-shadow: 0 4px 12px rgba(46, 221, 90, 0.18);
}

.btn-create-order:hover {
    background: linear-gradient(135deg, rgba(46, 221, 90, 0.98), rgba(34, 197, 94, 0.95));
    border-color: rgba(46, 221, 90, 0.5);
    transform: translateY(-2px);
    box-shadow: 0 8px 20px rgba(46, 221, 90, 0.28);
}

.btn-create-order:active {
    transform: translateY(0);
    box-shadow: 0 4px 10px rgba(46, 221, 90, 0.2);
}
...
.btn-back-to-dashboard:hover {
    ...
    transform: translateY(-2px);
    box-shadow: 0 6px 16px rgba(46, 221, 90, 0.2);
}
