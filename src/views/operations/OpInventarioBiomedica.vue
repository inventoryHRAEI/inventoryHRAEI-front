...
tienesdet         // Normalizar claves recibidas desde la API (acepta variantes con puntos, tildes o sin ellos)
        const normalizeKey = (s) => String(s || '').toUpperCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/[^A-Z0-9]/g, '')
        const findKey = (obj, candidates) => {
            ...
            // último intento: comparar normalizado contra todas las claves
            const candNorms = candidates.map(c => normalizeKey(c))
            for (const k of Object.keys(obj)) {
                const nk = normalizeKey(k)
                if (candNorms.includes(nk)) return obj[k]
            }
            return undefined
        }

        items.value = data.map(item => {
            const no = findKey(item, ['NO. INVENTARIO', 'NO INVENTARIO', 'claveHRAEI'])
            const equipo = findKey(item, ['EQUIPO', 'EQUIPO MEDICO', 'EQUIPO MÉDICO', 'name'])
            const marca = findKey(item, ['MARCA', 'brand'])
            const modelo = findKey(item, ['MODELO', 'model'])
            const serie = findKey(item, ['NUMERO DE SERIE', 'NÚMERO DE SERIE', 'SERIE', 'serie'])
            const unidad = findKey(item, ['UNIDAD MEDICA', 'UNIDAD MÉDICA', 'medicalUnit', 'UNIDAD'])
            const estatus = findKey(item, ['ESTADO', 'ESTATUS', 'status'])
            const fechaIngreso = findKey(item, ['FECHA INGRESO', 'FECHA_DE_INGRESO', 'intakeDate', 'FECHA DE ADQUISICION'])

            return {
                id: item.id || no || Math.random(),
                'No DE INVENTARIO': no || '',
                'EQUIPO MEDICO': equipo || '',
                'MARCA': marca || '',
                'MODELO': modelo || '',
                'NUMERO DE SERIE': serie || '',
                'UNIDAD MEDICA': unidad || '',
                'ESTATUS': estatus || '',
