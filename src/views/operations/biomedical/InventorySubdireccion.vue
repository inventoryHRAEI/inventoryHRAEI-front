    ...
    <div class="inventory-subdireccion">
        ...
        <div class="sub-header">
            <div class="header-left">
                ...
                <p class="subtitle">
                    ...
                    Gestión de existencias y control de almacén
                </p>
            </div>
        </div>
        ...
        <div v-else-if="filteredArticulos.length > 0">
            ...
            <template v-else-if="viewMode === 'table'">
                <BiomedicalEquipmentTableV2
                    :data="filteredArticulos"
                    :pageSize="15"
                    :show-all-columns="true"
                    rowClickAction="none"
                    :allowed-actions="['view','edit','kardex']"
                    entity-label="artículos"
                    @edit-item="handleEditItem"
                    @view-item="handleViewKardex"
...
import BiomedicalEquipmentTableV2 from '@/components/BiomedicalEquipmentTableV2.vue';
...
const fetchArticulos = async () => {
    ...
    try {
        console.log('[InventorySubdireccion] Iniciando fetch de /api/ops/stock-biomedica (full, dedupeByN)');
        // Request full DB rows and ask backend to dedupe by N to avoid duplicated N values in UI
        const response = await fetch(`/api/ops/stock-biomedica?full=1&dedupeByN=1&t=${Date.now()}`, { cache: 'no-store' });
        ...
        // We requested full data; trust backend sanitization/dedupe. Keep all returned rows.
        ...
        // Reordena: por N numérico ascendente (1,2,3...) — los N no numéricos van al final
        const parseNValue = (it) => {
            const raw = String(pickItemValue(it, ['N', 'n'], '') || '').trim();
            const parsed = parseInt(raw, 10);
            return Number.isFinite(parsed) ? parsed : Infinity;
        };

        const sortedItems = [...items].sort((a, b) => {
            const na = parseNValue(a);
            const nb = parseNValue(b);
            if (na !== nb) return na - nb;

            // Si ambos no tienen N numérico o son iguales, mantener la prioridad de items nuevos (si aplica)
            ...
            // Finalmente, determinismo por clave compuesta
