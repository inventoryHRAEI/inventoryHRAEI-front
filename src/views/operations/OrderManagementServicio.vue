<template>
    <div class="order-management-container">
                ...
                <div class="title-row">
                    ...
                    <details v-if="isAdmin" ref="archiveMenuRef" class="archive-menu">
                        <summary class="btn-reset-folios archive-summary" :class="{ 'viewing-archived': viewingArchived }" title="Opciones de archivo">
                            <svg v-if="!viewingArchived && !archivedStore.hasArchivedByType.servicio" xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                                <polyline points="21 8 21 21 3 21 3 8"></polyline>
                                <rect x="1" y="3" width="22" height="5"></rect>
                                <line x1="10" y1="12" x2="14" y2="12"></line>
                            </svg>
                            <svg v-else-if="!viewingArchived && archivedStore.hasArchivedByType.servicio" xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                        ...
                        <div class="archive-popover">
                            <button type="button" class="archive-action-item" :disabled="isResetting" @click="handleArchiveAction('servicio')">
                                Archivar y reiniciar folios
                            </button>
                            <button type="button" class="archive-action-item secondary" @click="openArchivedPanel('servicio')">
                                Ver archivadas ({{ archivedStore.archivedCounts.servicio || 0 }})
                            </button>
                        </div>
                    </details>
        ...
        <!-- Modal: Wizard de creación de orden de servicio -->
        <Teleport to="body">
                            ...
                            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                                ...
                                <line x1="6" y1="6" x2="18" y2="18"></line>
                                        ...
                                        <div class="wizard-modal-content">
                                            <OpServicio v-if="showWizardModal && wizardType === 'servicio'" :initialItems="wizardInitialItems" @created="onWizardCreated" @cancel="closeWizardImmediately" />
                                        </div>
                    </div>
                </div>
            </Transition>
        ...
        <ModalBase :open="showEditModal" @close="handleModalClose" @close-request="handleModalClose" :maxWidth="1100"
            ...
            <div class="om-edit-shell">
                ...
                <div v-if="activeTab !== 'main'" class="om-version-panel" role="tabpanel">
                     ...
                     <OpServicio v-if="isActiveTabEditable" :key="`edit-${selectedOrderId}-${activeTab}`" :modo="'editar'"
                     ...
                     <OpServicio v-else :key="`snap-${selectedOrderId}-${activeTab}`" :modo="'editar'"
                 ...
                 <div v-show="activeTab === 'main' && !branchTabs.length" role="tabpanel" class="om-main-panel">
                     <OpServicio :key="mainEditorKey" ref="OpServicioRef" :ordenId="selectedOrderId" :modo="'editar'"
...
const OpServicio = defineAsyncComponent(() => import('@/views/operations/OpServicioNew.vue'))
    ...
    try {
        ...
        const approved = new Set(
            (Array.isArray(requests) ? requests : [])
                .filter(req => String(req?.operation_type || '').toLowerCase() === 'servicio' && String(req?.status || '').toLowerCase() === 'approved')
                .map(req => normalizeOrderId(req?.operation_id))
                .filter(Boolean)
        )
        approvedEditOrderIds.value = approved
    } catch (error) {
        console.warn('[OrderManagementServicio] No se pudieron refrescar solicitudes de edición aprobadas', error)
    }
}
...
const handleRequestEdit = async (order) => {
  try {
    const res = await authedFetch(`/api/edit-requests`, {
      ...
      body: JSON.stringify({ operation_type: 'servicio', operation_id: order.folio })
    })
...
onMounted(() => {
    ...
    try { archivedStore.fetchArchived('servicio').catch(() => {}) } catch (e) {}
...
watch([showWizardModal, wizardType], ([open, type]) => {
    try {
        if (open) {
            saveWizardState({ routeName: route.name, type: type || 'servicio' })
        } else {
            clearWizardState()
        }
    } catch (e) {}
...
// Permisos del usuario (ya declarados)

// Wrapper para reiniciar folios desde la plantilla
const isResetting = ref(false)
async function confirmAndResetFolios(orderType = 'servicio') {
    if (isResetting.value) return
    isResetting.value = true
    try {
        const res = await resetFoliosHelper(orderType, { archivedPanelRef })
        try { await reloadOrdersFromServer() } catch (e) { console.warn('[OrderManagementServicio] reload after reset failed', e) }
        return res
    } catch (e) {
        console.warn('[OrderManagementServicio] reset folios failed', e)
    } finally {
...
const archiveButtonLabel = computed(() => {
    const count = Number(archivedStore.archivedCounts.servicio || 0)
...
function openArchivedPanel(orderType = 'servicio') {
    closeArchiveMenu()
...
async function handleArchiveAction(orderType = 'servicio') {
    closeArchiveMenu()
...
async function openDocumentModal(order) {
    ...
    const type = (order?.orderType || order?.type || order?.tipo || 'servicio').toString().toLowerCase()
...
async function fetchDocVersionsFor(folio, orderType = 'servicio') {
    ...
    try {
        const endpointPrefix = orderType || 'servicio'
...
async function forceGenerate() {
    ...
    docLoading.value = true
    try {
        const res = await authedFetch(`/api/ops/servicio/${encodeURIComponent(docTitle.value)}/pdfs/generate-force`)
...
async function generateAllVersions() {
    ...
    docLoading.value = true
    try {
        const res = await authedFetch(`/api/ops/servicio/${encodeURIComponent(docTitle.value)}/pdfs/generate-all-versions`)
        const json = await res.json().catch(() => ({}))
        if (!res.ok) {
...
async function fetchSignedDocuments(folio, orderType = 'servicio') {
    const endpointPrefix = orderType || 'servicio'
    ...
    uploadSignerNames.value = {}
        ; (async () => {
            try {
                ...
                const res = await authedFetch(`/api/ops/servicio/${fol}`)
                if (res.ok) {
                    const json = await res.json().catch(() => ({}))
    ...
    if (result.isConfirmed) {
        ...
        if (folio) {
            try {
                ...
                const delRes = await fetch(`/api/ops/servicio/${encodeURIComponent(folio)}`, { method: 'DELETE', headers })
                if (!delRes.ok) {
                    let body = null
...
async function submitUpload() {
    ...
    try {
        ...
        const res = await fetch(`/api/ops/servicio/${encodeURIComponent(uploadOrderFolio.value)}/pdfs/upload`, fetchOptions)
        ...
        if (orderIndex !== -1) {
            try {
                const res = await authedFetch(`/api/ops/servicio/${encodeURIComponent(uploadOrderFolio.value)}/signed-documents`)
                if (res.ok) {
                    const json = await res.json()
...
function selectDocVersion(v) {
    ...
    if (isMobileView.value) {
        ...
        currentPreviewUrl.value = v.previewUrl || `/api/ops/servicio/${encodeURIComponent(folio)}/pdfs/${v.id}/preview`
    }
...
const activeFiltersList = computed(() => {
    ...
    if (filterMotivoActive.value) list.push({ key: 'motivo', label: 'Motivo de servicio', type: 'select', bindings: { modelValue: filterMotivo, 'onUpdate:modelValue': val => filterMotivo.value = val, class: 'control filter-input' } })
...
function goToCreateOrder() {
    console.debug('[OrderManagementServicio] opening create order modal')
    showWizardModal.value = true
}

// Manejar cuando se crea una orden exitosamente desde el wizard
async function onWizardCreated() {
    console.debug('[OrderManagementServicio] order created, closing modal and refreshing')
...
function discardCurrentWizardDraft() {
    ...
    try { deleteDraftRemote(wizardType.value) } catch (e) { console.warn('[OrderManagementServicio] failed to delete remote draft for', wizardType.value, e) }
}
...
async function loadOrderHistoryAndVersions(folio) {
    ...
    try {
        const [histRes, versRes] = await Promise.all([
            authedFetch(`/api/ops/servicio/${encodeURIComponent(f)}/history`, { cache: 'no-store' }),
            authedFetch(`/api/ops/servicio/${encodeURIComponent(f)}/versions?limit=6`, { cache: 'no-store' })
        ])
...
async function persistEditedOrder(payload) {
    try {
        ...
        try {
            const curRes = await fetch(`/api/ops/servicio/${encodeURIComponent(String(folio))}`, { cache: 'no-store' })
            if (curRes && curRes.ok) {
        ...
        const res = await fetch(`/api/ops/servicio/${encodeURIComponent(String(folio))}`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
        ...
        try {
            const genRes = await fetch(`/api/ops/servicio/${encodeURIComponent(folio)}/pdfs/generate-force`)
            if (genRes && genRes.ok) {
...
async function reloadOrdersFromServer() {
     loading.value = true
     try {
         const res = await fetch('/api/ops/servicio/list', { cache: 'no-store' })
        if (!res.ok) {
            allOrders.value = []
            return
        }
...
async function loadDocumentCountsForOrders() {
    ...
    const documentCountPromises = allOrders.value.map(async (order) => {
        try {
            const res = await fetch(`/api/ops/servicio/${encodeURIComponent(order.folio)}/signed-documents`)
            if (res.ok) {
                const json = await res.json()
                const count = (json && json.documents && Array.isArray(json.documents)) ? json.documents.length : 0
...
// duplicate deleteOrder removed; use earlier cancel/delete implementation above
...
async function deleteMultipleOrders(orderIds) {
    ...
    const results = await Promise.allSettled(Object.keys(folioMap).map(async (folio) => {
        try {
            const res = await fetch(`/api/ops/servicio/${encodeURIComponent(folio)}`, { method: 'DELETE', headers })
...
const orderSearchFilters = {
    ...
    suggestionsByField: computed(() => {
        ...
        const getItems = (order) => Array.isArray(order.equiposSalida) ? order.equiposSalida : Array.isArray(order.equiposEntrada) ? order.equiposEntrada : []
        ...
        addBucket('motivo', orders.map(order => order.motivoServicio || order.motivoEntrada || order.motivoSalida || order.motivoResguardo || order.motivo))
