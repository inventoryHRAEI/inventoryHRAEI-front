<template>
    <div :class="{ 'opsalida-readonly': isReadOnly }" :aria-readonly="isReadOnly ? 'true' : 'false'"
        ...
        <FormShell>
            ...
            <template #title v-else>
                <div class="edit-title-header">
                    ...
                    <h2 class="edit-title">Editando orden de salida</h2>
            ...
            <template #body>
                ...
                <div v-else class="op-card insumos op-salida-form" ref="rootRef">
                    <form @submit.prevent="onSubmit" class="form-grid" id="salida-form" novalidate>
                        <div class="section-card combined-card observaciones-support op-enhanced">
                            ...
                            <div class="section-grid combined">
                                ...
                                <div v-if="!isBaseReplaced('nombreSolicitante')" :class="['field', diffFieldClass('nombreSolicitante')]">
                                    ...
                                    <input class="control" v-model.trim="form.nombreSolicitante" :readonly="!isAdmin"
                                ...
                                <div v-if="!isBaseReplaced('folio')" :class="['field', diffFieldClass('folio')]">
                                    ...
                                    <div style="display:flex; flex-direction:column; gap:6px">
                                        ...
                                        <small class="hint">Formato requerido: <code>S-001</code></small>
                                    </div>
                                </div>
                        ...
                        <!-- Motivo y Descripción de Salida -->
                        <div class="section-card combined-card">
                            <div class="section-head">
                                <div class="section-title-with-icon">
                                    ...
                                    <h4>Motivo y Descripción de Salida</h4>
                                </div>
                                <small class="hint">Especifica el motivo y una descripción de la salida</small>
                            </div>
                            <div class="section-grid combined">
                                <div v-if="!isBaseReplaced('motivoSalida')" :class="['field', diffFieldClass('motivoSalida')]" style="grid-column: span 6;">
                                    <label>{{ getBaseLabel('motivoSalida', 'Motivo de Salida') }}</label>
                                    <CustomSelect v-model="form.motivoSalida" :options="motivoSalidaOptionsComputed"
                                        :placeholder="getBasePlaceholder('motivoSalida', 'Seleccionar motivo')" />
                                </div>

                                <div v-if="!isBaseReplaced('otroMotivo') && form.motivoSalida === 'otro'"
                                    :class="['field', diffFieldClass('otroMotivo')]" style="grid-column: span 6;">
                                    <label>{{ getBaseLabel('otroMotivo', 'Especifique Motivo de Salida') }}</label>
                                ...
                                <div v-if="!isBaseReplaced('descripcion')" :class="['field', diffFieldClass('descripcion')]" style="grid-column: 1 / -1;">
                                    <label>{{ getBaseLabel('descripcion', 'Descripción de Salida') }}</label>
                        ...
                        <!-- Equipo Médico, Accesorio o Consumible que Sale -->
                        <div class="section-card combined-card">
                            <div class="section-head">
                                <div class="section-title-with-icon">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"
                                    ...
                                    <h4>Equipo Médico, Accesorio o Consumible que Sale</h4>
                                </div>
                            ...
                            <div v-if="form.equiposSalida.length > 0" class="items-list"
                                    ...
                                    <h5
                                        ...
                                        <span
                                            style="display: inline-block; background: rgb(59, 130, 246); color: white; padding: 2px 10px; border-radius: 12px; font-size: 0.85rem; margin-left: 8px;">{{
                                                form.equiposSalida.length }}</span>
                                    </h5>
                                    <div
                            ...
                            <p v-if="form.equiposSalida.length === 0" class="section-empty-hint"
                        ...
                        <div class="section-card combined-card">
                            ...
                            <div class="section-grid combined">
                                ...
                                <div v-if="!isBaseReplaced('nombreIngeniero')" :class="['field', 'ing-res', diffFieldClass('nombreIngeniero')]">
                                    ...
                                    <input class="control" v-model.trim="form.nombreIngeniero" readonly
                    ...
                    <div v-if="!isReadOnly" class="form-actions">
                        ...
                        <button class="btn primary save-btn" type="submit" form="salida-form"
...
const schemaSections = [
    ...
    { id: 'equipos', title: 'Equipo Médico, Accesorio o Consumible que Sale' },
...
// Force engineer name to authenticated user (do not allow editing)
...
onMounted(() => {
    ...
    if (authName) form.nombreIngeniero = authName
})
...
function applySnapshotToForm(snapshot) {
    ...
    form.motivoSalida = orden.motivo_salida || ''
...
// Opciones del select de Motivo de Salida
// Opciones del select de Motivo de Salida (importadas desde data)
// motivoSalidaOptions importado arriba

// Sanitizar folio a formato S-XXX
...
async function generarPdfSalida(payloadParam) {
    ...
    try {
        ...
        if (res && res.ok) {
            ...
            let filename = `salida_${payloadToSend.folio || 'sin_folio'}.pdf`
...
const canAddItem = computed(() => {
    ...
    // Para estos tipos requerimos que exista al menos una unidad y referencia en cada unidad
    if (['equipo-medico', 'mobiliario', 'accesorio', 'consumible', 'refaccion'].includes(newItem.tipo)) {
        if (!Array.isArray(newItem.unidades) || newItem.unidades.length === 0) return false
        return newItem.unidades.every(u => (u.referencia || '').trim().length > 0)
    }

    // Fallback: descripción obligatoria y referencia
    return (newItem.descripcion || '').trim().length > 0 && (newItem.referencia || '').trim().length > 0
})
    ...
    if (['accesorio', 'consumible', 'refaccion'].includes(itemData.tipo)) {
        if (Array.isArray(itemData.unidades) && itemData.unidades.length > 0) {
            ...
            for (const unidad of itemData.unidades) {
                ...
                form.equiposSalida.push(single)
            }
        } else {
        ...
        // Crear una salida por cada unidad (cantidad=1) y conservar el orden.
...
const eliminarItem = (targetItem) => {
    const targetIndex = form.equiposSalida.findIndex((entry) => entry === targetItem)
    if (targetIndex === -1) {
        return
    }
    form.equiposSalida.splice(targetIndex, 1)
...
const onUnitTrashDone = (itemObj, unidadObj) => {
    try {
        ...
        setTimeout(() => {
            ...
            const parentIndex = form.equiposSalida.findIndex(e => e === itemObj)
            if (parentIndex === -1) {
                exitingUnits.value = exitingUnits.value.filter(u => u !== unidadObj)
                return
            }
            const unitIndex = form.equiposSalida[parentIndex].unidades.findIndex(u => u === unidadObj)
            if (unitIndex === -1) {
                exitingUnits.value = exitingUnits.value.filter(u => u !== unidadObj)
                return
            }
            ...
            // perform removal
            form.equiposSalida[parentIndex].unidades.splice(unitIndex, 1)
            ...
            if (!form.equiposSalida[parentIndex].unidades || form.equiposSalida[parentIndex].unidades.length === 0) {
                ...
                if (!exitingItems.value.includes(itemObj)) {
                    ...
                    setTimeout(() => {
                        const idx = form.equiposSalida.findIndex(e => e === itemObj)
                        if (idx !== -1) {
...
const saveEditedUnit = () => {
    try {
        ...
        const parentIndex = form.equiposSalida.findIndex(e => e === parentItem)
        if (parentIndex === -1) {
        ...
        form.equiposSalida[parentIndex].unidades[unitIndex] = updatedUnit
...
const onCancel = async () => {
    ...
    if (props.modo === 'editar') {
        ...
        if (result.isConfirmed) {
            ...
            try {
                await navigateAndRefresh(router, { name: 'order-management-salida' })
            } catch {
                try { await navigateAndRefresh(router, '/op/order-management-salida') } catch { }
            }
        }
    }
}
...
const goToOrderManagement = () => {
    ...
    navigateAndRefresh(router, { name: 'order-management-salida' })
}
...
async function generarExcelSalida(payloadParam) {
    ...
    try {
        const res = await authedFetch('/api/ops/salida-excel-com', {
            method: 'POST',
            body: JSON.stringify(payloadToSend)
        })
        if (res && res.ok) {
            ...
            const contentDisposition = res.headers.get('content-disposition') || ''
            let filename = 'salida_plantilla_copia.xlsx'
            const fnMatch = contentDisposition.match(/filename="?([^";]+)"?/) || contentDisposition.match(/filename\*=UTF-8''([^;\n]+)/)
            if (fnMatch) filename = decodeURIComponent(fnMatch[1])
            const blob = await res.blob()
            saveAs(blob, filename)
    ...
    try {
        ...
        const plantillaUrl = new URL('../../plantillas/salida_plantilla.xlsx', import.meta.url).href
        ...
        const motiveOptions = (motivoSalidaOptionsComputed.value || []).filter(o => o && o.value)
        const indiceMotivo = motiveOptions.findIndex(o => o.value === form.motivoSalida)
        ...
        const equiposData = form.equiposSalida.filter(item => item.tipo === 'equipo-medico' || item.tipo === 'mobiliario')
        ...
        const accesoriosData = form.equiposSalida.filter(item => item.tipo === 'accesorio')
        ...
        const consumiblesData = form.equiposSalida.filter(item => item.tipo === 'consumible')
        ...
        const refaccionesData = form.equiposSalida.filter(item => item.tipo === 'refaccion')
        ...
        // Las filas 7-14 contienen los motivos de salida y NO deben tener fondo azul
        ...
        saveAs(blob, `salida_${form.folio || 'sin-folio'}_${Date.now()}.xlsx`)
...
async function onSubmit() {
    ...
    const equipos = form.equiposSalida.filter(i => i.tipo === 'equipo-medico' || i.tipo === 'mobiliario')
    const accesorios = form.equiposSalida.filter(i => i.tipo === 'accesorio')
    const consumibles = form.equiposSalida.filter(i => i.tipo === 'consumible')
    const refacciones = form.equiposSalida.filter(i => i.tipo === 'refaccion')
    ...
    const motivoLabel = (() => {
        if (form.motivoSalida === 'otro') return `OTRO: ${escapeHtml(form.otroMotivo || '')}`
        const opt = motivoSalidaOptionsComputed.value.find(o => o.value === form.motivoSalida)
  ...
  `
    ...
    try {
        ...
        if (props.modo === 'editar') {
            ...
            console.log('[OpSalida] Emitting actualizado payload.signatures:', payload.signatures)
        ...
        if (!res.ok) {
            ...
            await generarPdfSalida(payload)
            clearForm()
            // Navegar a Order Management para que la tabla se actualice con la nueva orden
            try {
                await router.replace({ name: 'order-management-salida', query: { t: Date.now() } })
            ...
            try {
                ...
                if (currentName !== 'order-management-salida') {
                    const ts = Date.now()
                    // perform SPA navigation replace and emit force-recreate to remount components without reloading the page
                    try { router.replace({ path: '/op/order-management-salida', query: { t: ts } }).catch(()=>{}) } catch (e) { /* ignore */ }
...
const loadOrderData = async () => {
    ...
    try {
        ...
        const res = await fetch(`/api/ops/salida/${encodeURIComponent(ordenIdStr)}?t=${Date.now()}`, { cache: 'no-store' })
        ...
        form.motivoSalida = data.motivo_salida || data.motivoSalida || ''
        ...
        if (response.items && Array.isArray(response.items)) {
            ...
            for (const item of response.items) {
                ...
                if (item.unidades && Array.isArray(item.unidades) && item.unidades.length) {
                    if (['accesorio', 'consumible', 'refaccion'].includes(tipo)) {
                        // Crear una salida por cada unidad para preservar orden
            ...
            form.equiposSalida = loaded
            console.info('[OpSalida] Loaded items (preserving order):', loaded.map((it, idx) => ({ idx, tipo: it.tipo, descripcion: it.descripcion, cantidad: it.cantidad })))
            ...
            form.equiposSalida = Array.isArray(data.equiposSalida) ? data.equiposSalida : []
        }
        ...
        if (data.observaciones_img_path) {
            ...
            if (imgPath && imgPath.length > 0 && imgPath !== 'null' && imgPath !== 'undefined') {
                ...
                console.log('[OpSalida] Imagen cargada:', { name: form.observacionesImg.name, url: finalUrl })
            } else {
                form.observacionesImg = null
                console.log('[OpSalida] observaciones_img_path vacío o inválido')
            }
        } else {
            form.observacionesImg = null
            console.log('[OpSalida] Sin observaciones_img_path')
        }
...
onMounted(async () => {
    ...
    await initSuggestions().catch(err => {
        console.warn('[OpSalida] Error initializing suggestions:', err)
    })
    ...
    if (props.modo === 'editar') {
        ...
        try {
            ...
            const loggedUser = JSON.parse(localStorage.getItem('user') || 'null') || { nombre: localStorage.getItem('nombre') }
            form.nombreSolicitante = form.nombreSolicitante || loggedUser.nombre || ''
            form.nombreIngeniero = loggedUser.nombre || ''
            form.items = []
            syncItemsToCantidad()
        }
...
onMounted(() => {
    try { console.debug('[OpSalida] mounted route:', router.currentRoute && router.currentRoute.value ? router.currentRoute.value.fullPath : '(unknown)') } catch {}
    try { window.dispatchEvent(new CustomEvent('route:mounted', { detail: { name: router.currentRoute && router.currentRoute.value ? router.currentRoute.value.name : '', path: router.currentRoute && router.currentRoute.value ? router.currentRoute.value.fullPath : '' } })); console.debug('[OpSalida] dispatched route:mounted') } catch {}
...
onBeforeUnmount(() => {
    try { console.debug('[OpSalida] beforeUnmount route:', router.currentRoute && router.currentRoute.value ? router.currentRoute.value.fullPath : '(unknown)') } catch {}
...
.opsalida-readonly :deep(.field.diff-yellow),
.opsalida-readonly :deep(.field.diff-red),
.opsalida-readonly :deep(.field.diff-green),
.opsalida-readonly :deep(.item-row.diff-yellow),
.opsalida-readonly :deep(.item-row.diff-red),
.opsalida-readonly :deep(.item-row.diff-green) {
    position: relative;
}

.opsalida-readonly :deep(.field.diff-yellow::before),
.opsalida-readonly :deep(.field.diff-red::before),
.opsalida-readonly :deep(.field.diff-green::before),
.opsalida-readonly :deep(.item-row.diff-yellow::before),
.opsalida-readonly :deep(.item-row.diff-red::before),
.opsalida-readonly :deep(.item-row.diff-green::before) {
    content: '';
    position: absolute;
    inset: -6px;
    border-radius: 14px;
...
.opsalida-readonly :deep(input),
.opsalida-readonly :deep(textarea),
.opsalida-readonly :deep(button),
.opsalida-readonly :deep(select) {
...
.op-salida-form input[type="date"] {
    color-scheme: dark !important;
}

/* CSS específico y directo para el calendario */
.op-salida-form input[type="date"]::-webkit-calendar-picker-indicator {
...
:where(.op-salida-form) input[type="date"] {
    color-scheme: dark;
}
...
/* ========= Mejoras visuales adicionales específicas para OpSalida ========= */
.op-salida-form {
    padding: 8px;
}

.op-salida-form .section-card {
...
.op-salida-form .section-card:hover {
    transform: translateY(-6px);
    box-shadow: 0 22px 44px rgba(0, 6, 22, 0.72);
}

.op-salida-form .section-head h4 {
    color: #eef6ff
}

.op-salida-form .section-head svg {
    background: rgba(255, 255, 255, 0.03);
    padding: 6px;
    border-radius: 8px
}

.op-salida-form .field label {
    color: rgba(230, 238, 248, 0.86);
    font-weight: 700;
    font-size: 0.86rem
}

.op-salida-form .control {
    background: rgba(255, 255, 255, 0.03);
...
:where(.op-salida-form) .control::placeholder {
