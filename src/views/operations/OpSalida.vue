          ...
          <div class="form-footer combined-card">
            <div class="form-actions">
              ...
              >
                ...
                {{ isSubmitting ? 'Guardando...' : 'Crear Orden de Salida' }}
              </button>
            </div>
          </div>
        </form>
...
const motivoSalidaOptions = [
  ...
  { value: 'devolucion-comodato', label: 'DEVOLUCIÓN COMODATO' },
...
const agregarItem = () => {
  ...
  form.equiposSalida.push(itemToAdd)
...
const onTrashDone = (item) => {
  ...
  if (idx > -1) form.equiposSalida.splice(idx, 1)
}
...
const isFormValid = computed(() => {
  ...
  if (form.equiposSalida.length === 0) return false
  return true
})
...
const onSubmit = async () => {
  ...
  try {
    const result = await Swal.fire({
      ...
      text: 'Se generará una nueva orden de salida con los datos ingresados.',
      icon: 'question',
    ...
    notifier.success('Orden de salida creada correctamente')
    router.push({ name: 'admin-dashboard' })
    
  } catch (error) {
    console.error(error)
    notifier.error('Error al crear la orden de salida')
  } finally {
...
/* Overrides específicos para salida */
.op-card.insumos {
  max-width: 100%;
}
