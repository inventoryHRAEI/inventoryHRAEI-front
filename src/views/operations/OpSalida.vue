<template>
  <div>
    <FormShell>
      <template #title>Órdenes de Salida</template>
  ...
  <div class="op-card insumos" ref="rootRef">
        <form @submit.prevent="onSubmit" class="form-grid" id="salida-form" novalidate>
          ...
          <div class="section-card combined-card">
            <div class="section-head">
              ...
              <small class="hint">Agrega uno o más elementos que salen</small>
            </div>
              ...
              <div class="add-item-form__inner">
                ...
                <div class="add-item-controls">
                  <div class="field field--tipo" :class="{ 'is-shifted': newItem.tipo }" :style="{ position: 'relative', zIndex: 1001 }">
                    <label class="form-label form-label--center">¿Qué sale?</label>
                    <CustomSelect 
            ...
            <div v-if="form.equiposSalida.length > 0" class="items-list" style="display: flex; flex-direction: column; gap: 16px;">
              <div style="display: flex; align-items: center; gap: 10px; margin-bottom: 8px;">
                ...
                <h5 style="margin: 0; font-size: 1.05rem; color: rgba(15, 23, 42, 0.95); font-weight: 700;">
                  Items Agregados
                  <span style="display: inline-block; background: rgb(59, 130, 246); color: white; padding: 2px 10px; border-radius: 12px; font-size: 0.85rem; margin-left: 8px;">{{ form.equiposSalida.length }}</span>
                </h5>
                <div style="flex: 1; height: 2px; background: linear-gradient(to left, rgb(59, 130, 246), transparent);"></div>
              </div>
              
              <div v-for="(item, index) in form.equiposSalida" :key="index" :class="['item-row', { 'item-row-exit': exitingItems.includes(item) }]">
                <div class="item-head">
            ...
            <p v-if="form.equiposSalida.length === 0" style="color: rgba(100, 116, 139, 0.7); font-style: italic; text-align: center; margin: 20px 0;">
        ...
        <div class="form-actions">
          ...
          <button class="btn primary save-btn" type="submit" form="salida-form" :disabled="loading || !isValid">
...
// Opciones del select de tipo de salida (mismos tipos que en entrada)
...
const agregarItem = () => {
  ...
  if (newItem.tipo === 'equipo-medico' || newItem.tipo === 'mobiliario') {
    ...
    form.equiposSalida.push(itemData)
  } else {
...
<style scoped>
                ...
                /* ...remaining identical CSS omitted for brevity in patch... */
